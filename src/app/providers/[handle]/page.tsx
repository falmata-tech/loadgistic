...
export default async function ProviderPage({params,searchParams}:{params:Promise<{handle:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const provider:any=await getPublicProvider(handle,{truckPage:(await searchParams).truckPage});
  ...
  return <><PublicHeader/><main className="provider-microsite provider-microsite-v2">
    ...
    <section className="provider-site-hero"><div className="container provider-site-hero-grid">
      ...
      <div className="provider-site-identity"><span className="provider-handle">{provider.provider_kind_label} · /@{provider.handle}</span><h1>{provider.name}</h1><p>{provider.headline||'Professional road-freight services across Ethiopia'}</p><div className="provider-hero-facts">{provider.city?<span><MapPin aria-hidden="true"/>{provider.city}</span>:null}<span><Truck aria-hidden="true"/>{provider.fleet_page.total} active {provider.fleet_page.total===1?'truck':'trucks'}</span><span><Star aria-hidden="true"/>{provider.review_count?`${provider.average_rating} · ${provider.review_count} verified shipment ${provider.review_count===1?'review':'reviews'}`:'New to Loadgistic'}</span></div></div>
    ...
    <div className="container provider-site-content">
      <ProviderFleetShowcase key={provider.fleet_page.page} providerName={provider.name} trucks={provider.trucks} total={provider.fleet_page.total}/>
      <Pagination path={`/@${provider.handle}`} query={{}} pageParam="truckPage" page={provider.fleet_page.page} pageCount={provider.fleet_page.pageCount} total={provider.fleet_page.total}/>
