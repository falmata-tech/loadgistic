...
export default async function TrackPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <><PublicHeader/><main className="public-app-page tracking-public-workspace"><div className="container tracking-public-container">
    ...
    <div className="tracking-mode-banner"><strong>{shipment.tracking_mode==='LOCATION_AND_STATUS'?'Location and status updates':'Status updates'}</strong><span>Updates come from the transporter. Confirm cargo, pickup, delivery, and timing directly with them.{shipment.tracking_mode==='LOCATION_AND_STATUS'?' Location updates require the Driver’s open, visible Tracking screen and pause when it closes or the phone locks.':''}</span></div>
