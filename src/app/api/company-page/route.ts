...
export async function POST(request:NextRequest){
 ...
 try{updateCompanyPage(user,{headline:text(form,'headline'),about:text(form,'about'),services:text(form,'services'),basePlaceRef:text(form,'basePlaceRef'),basePlaceLabel:text(form,'basePlaceLabel'),contactPhone:text(form,'contactPhone'),contactWhatsapp:text(form,'contactWhatsapp'),contactEmail:text(form,'contactEmail'),contactWebsite:text(form,'contactWebsite'),showContactPhone:checked(form,'showContactPhone'),showContactWhatsapp:checked(form,'showContactWhatsapp'),showContactEmail:checked(form,'showContactEmail'),showContactWebsite:checked(form,'showContactWebsite'),themePrimary:text(form,'themePrimary'),themeAccent:text(form,'themeAccent'),youtubeVideoId:text(form,'youtubeVideoId'),published:checked(form,'published')});return redirectWith(request,'/app/company-page','success','Provider page updated.');}
