...
export async function POST(request:NextRequest){
 ...
 const originRefs=form.getAll('routeOriginPlaceRef').map(String); const destinationRefs=form.getAll('routeDestinationPlaceRef').map(String);
 ...
 try{updateCompanyPage(user,{headline:text(form,'headline'),about:text(form,'about'),services:text(form,'services'),basePlaceRef:text(form,'basePlaceRef'),basePlaceLabel:text(form,'basePlaceLabel'),routes:origins.map((origin,index)=>({origin,originPlaceRef:originRefs[index]||'',destination:destinations[index]||'',destinationPlaceRef:destinationRefs[index]||''})),serviceAreas:areaLabels.map((placeLabel,index)=>({placeRef:areaRefs[index]||'',placeLabel,radiusKm:areaRadii[index]||''})),contactPhone:text(form,'contactPhone'),showContactPhoneOnLoads:checked(form,'showContactPhoneOnLoads'),contactEmail:text(form,'contactEmail'),published:checked(form,'published')});return redirectWith(request,'/app/company-page','success','Profile updated.');}
