...
export async function POST(request:NextRequest){
  const rate=checkRateLimit(requestKey(request,'guest-support-access'),8,60_000);
  ...
  try{const result=verifyGuestSupportAccess(text(form,'email'),text(form,'code'));await setGuestSupportSession(result.conversationId,result.emailDigest);return redirectWith(request,`/help/${result.conversationId}`,'success','Conversation opened.');}
