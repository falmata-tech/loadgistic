...
export async function POST(request:NextRequest){const user=await getCurrentUser();if(!user)return NextResponse.redirect(new URL('/login',request.url),303);const form=await request.formData();const back=destination(form);try{if(text(form,'action')==='REMOVE')removeRecurringCorridor(user,text(form,'id'));else addRecurringCorridor(user,{geometry:text(form,'geometry'),origin:text(form,'origin'),originPlaceRef:text(form,'originPlaceRef'),destination:text(form,'destination'),destinationPlaceRef:text(form,'destinationPlaceRef'),center:text(form,'center'),centerPlaceRef:text(form,'centerPlaceRef'),radiusKm:text(form,'radiusKm')});return redirectWith(request,back,'success',text(form,'action')==='REMOVE'?'Recurring signal removed.':'Recurring signal added.');}catch(error){return redirectWith(request,back,'error',errorMessage(error));}}
