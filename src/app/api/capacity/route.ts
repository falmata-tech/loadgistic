...
export async function POST(request:NextRequest){
 ...
 }catch(error){if(upload?.path)try{await removePrivateUpload(upload.path);}catch{/* orphan cleanup can be retried operationally */}return redirectWith(request,returnTo,'error',errorMessage(error));}
}
