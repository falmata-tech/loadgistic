...
export async function POST(request:NextRequest){
 ...
 try{
   ...
   publishCapacity(user,{vehicleId,status:text(form,'status'),availablePercent:text(form,'availablePercent'),availableAgainDate:text(form,'availableAgainDate'),acceptedLoads:text(form,'acceptedLoads'),movementScope:text(form,'movementScope')||'INTERCITY',localPlaceRef:text(form,'deviceLocalPlaceRef')||text(form,'localPlaceRef'),localPlaceLabel:text(form,'localPlaceLabel'),localRadiusKm:text(form,'localRadiusKm'),locationArea:text(form,'locationArea'),locationPlaceRef:text(form,'locationPlaceRef'),approximateLat:text(form,'approximateLat'),approximateLng:text(form,'approximateLng'),locationPrecisionKm:text(form,'locationPrecisionKm'),locationSource:text(form,'locationSource'),origin,originPlaceRef:text(form,'originPlaceRef'),destination,destinationPlaceRef:text(form,'destinationPlaceRef'),currentRouteOrigin:text(form,'currentRouteOrigin'),currentOriginPlaceRef:text(form,'currentOriginPlaceRef'),currentRouteDestination:text(form,'currentRouteDestination'),currentDestinationPlaceRef:text(form,'currentDestinationPlaceRef'),currentRouteDate:text(form,'currentRouteDate'),travelDate:text(form,'travelDate'),plannedSpaceStatus:text(form,'plannedSpaceStatus'),nextAvailable:text(form,'nextAvailable'),visibility:text(form,'visibility'),openToContractLanes:checked(form,'openToContractLanes'),acceptsMultiPick:checked(form,'acceptsMultiPick'),acceptsMultiDrop:checked(form,'acceptsMultiDrop')},upload);
