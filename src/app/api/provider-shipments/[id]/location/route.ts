...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const result=updateProviderShipmentLocation(user,id,{locationArea:String(input.locationArea||''),approximateLat:String(input.approximateLat||''),approximateLng:String(input.approximateLng||''),locationPrecisionKm:String(input.locationPrecisionKm||''),locationSource:String(input.locationSource||'')});
