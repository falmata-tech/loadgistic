...
export default async function CompanyPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  if (!company) notFound();
  const canReceiveRequests = ['PARCEL_OPERATOR','TRANSPORT_COMPANY','INDEPENDENT_PROVIDER'].includes(company.type);
  const providerRef = `${company.page_kind === 'provider' ? 'profile' : 'org'}:${company.id}`;
  const requestHref = user ? `/app/shipments/new?provider=${providerRef}` : '/login';
