...
export default async function HomePage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="page"><PageHeader icon={LayoutDashboard} title={greeting} subtitle="Choose your next task."/><Flash error={query.error} success={query.success}/>
    ...
    <div className="two-col"><section className="card"><div className="page-header" style={{marginBottom:12}}><div><h2 className="panel-heading"><MapPin aria-hidden="true"/>Recent tracking</h2><p className="page-subtitle">Shipments involving this workspace.</p></div><Link href="/app/shipments" className="button secondary small"><Eye aria-hidden="true"/>View all</Link></div>{data.recent.length?<div className="list">{data.recent.map((item:any)=><Link href={`/app/shipments/${item.id||item.code}`} className="list-row compact" key={item.id||item.code}><div><strong>{item.code}</strong><div className="meta">{item.title}</div></div><div className="route">{item.origin}<span>→</span>{item.destination}</div><StatusPill status={item.operational_status}/></Link>)}</div>:<div className="empty-state">No tracking yet.</div>}</section>
