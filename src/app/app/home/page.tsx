...
export default async function HomePage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const greeting=user.organization_name||user.provider_business_name||user.name;
  return <div className="page"><PageHeader icon={LayoutDashboard} title={greeting} subtitle={<Text message="Manage today's transport operations."/>}/><Flash error={query.error} success={query.success}/>
    ...
    <div className="two-col"><section className="card"><div className="page-header" style={{marginBottom:12}}><div><h2 className="panel-heading"><MapPin aria-hidden="true"/><Text message="Recent Tracking"/></h2><p className="page-subtitle"><Text message="Latest shipment activity."/></p></div><Link href="/app/provider-shipments" className="button secondary small"><Eye aria-hidden="true"/><Text message="View all"/></Link></div>{data.recent.length?<div className="list">{data.recent.map((item:any)=><Link href={`/app/provider-shipments/${item.id}`} className="list-row compact" key={item.id}><div><strong>{item.code}</strong><div className="meta">{item.cargo_summary||item.title}</div></div><div className="route">{item.origin}<span>→</span>{item.destination}</div><StatusPill status={item.operational_status}/></Link>)}</div>:<div className="empty-state"><Text message="No tracking activity yet."/></div>}</section>
    <aside className="stack"><section className="card"><h3><ListChecks aria-hidden="true"/><Text message="Operating priorities"/></h3><p className="muted">{user.role==='TRANSPORTER'||user.role==='DRIVER'?<Text message="Keep truck availability, your public profile, and active tracking records current."/>:user.role==='ADMIN'?<Text message="Review the queues requiring platform action."/>:<Text message="Review current marketplace activity."/>}</p></section>{data.notifications?.length?<section className="card"><h3><BadgeCheck aria-hidden="true"/><Text message="Notifications"/></h3><div className="stack">{data.notifications.slice(0,4).map((n:any)=><div key={n.id}><strong>{n.title}</strong><div className="meta">{n.body}</div></div>)}</div></section>:null}</aside></div>
  </div>;
}
