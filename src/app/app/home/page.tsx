...
export default async function HomePage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="page"><PageHeader icon={LayoutDashboard} title={greeting} subtitle="Choose your next task."/><Flash error={query.error} success={query.success}/>
    ...
    <div className="two-col"><section className="card"><div className="page-header" style={{marginBottom:12}}><div><h2 className="panel-heading"><MapPin aria-hidden="true"/>Recent Tracking</h2><p className="page-subtitle">Tracking sessions created by this provider.</p></div><Link href="/app/provider-shipments" className="button secondary small"><Eye aria-hidden="true"/>View Tracking</Link></div>{data.recent.length?<div className="list">{data.recent.map((item:any)=><Link href={`/app/provider-shipments/${item.id}`} className="list-row compact" key={item.id}><div><strong>{item.code}</strong><div className="meta">{item.cargo_summary||item.title}</div></div><div className="route">{item.origin}<span>→</span>{item.destination}</div><StatusPill status={item.operational_status}/></Link>)}</div>:<div className="empty-state">No Tracking sessions yet.</div>}</section>
