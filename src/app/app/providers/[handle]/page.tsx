...
export default async function CompanyPage({ params,searchParams }: { params: Promise<{ handle: string }>; searchParams:Promise<Record<string,string|undefined>> }) {
  ...
  return <div className="page"><Flash error={query.error} success={query.success}/>
    <div className="company-hero"><div className="company-logo">{initials}</div><div><div className="profile-title-line"><h1 className="page-title">{company.name}</h1>{company.verified?<StatusPill status="Approved"/>:null}</div><p className="page-subtitle">{company.headline}</p><div className="meta icon-meta"><MapPin aria-hidden="true"/>{company.city||'Location not added'} · {isBusiness?'Business':String(company.type).replaceAll('_',' ')}</div><VerificationBadges badges={company.verification_badges}/></div><div className="hero-actions">{canSendProviderRequest?<Link className="button" href={`/app/shipments/new?provider=${providerRef}`}>Send request</Link>:null}{canSelectReceiver?<Link className="button" href={`/app/shipments/new?receiver=${company.id}`}>Create load together</Link>:null}{!isOwnProfile&&company.routes.length?<Link className="button secondary icon-button-label" href={`/app/providers/${handle}?compare=routes#route-coverage`}><GitCompareArrows aria-hidden="true"/>Compare routes</Link>:null}<NetworkActions state={networkState} targetKind={company.page_kind==='provider'?'profile':'org'} targetId={company.id} returnTo={`/app/providers/${handle}`}/></div></div>
