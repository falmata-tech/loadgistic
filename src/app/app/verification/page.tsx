...
export default async function VerificationPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <div className="page">
    <PageHeader icon={BadgeCheck} title={<Text message="Verification"/>} subtitle={<Text message="Build confidence in your transport service with optional document review."/>}/>
    <Flash error={query.error} success={query.success}/>
    <div className="two-col">
      <div className="stack">
        <section className="card"><div className="section-heading-icon"><BadgeCheck aria-hidden="true"/><div><h2><Text message="Badge status"/></h2><p className="meta"><Text message="Open any badge for its scope, review date, and expiry details."/></p></div></div>
        ...
        <VerificationForm subjects={formSubjects} initialTruckId={query.truck}/>
      </div>
      <aside className="card verification-history"><h2><Text message="Request history"/></h2><div className="stack">{requestResult.items.map((request:any)=><div className="request-history-row" key={request.id}><div><strong>{request.document_name}</strong><div className="meta"><Text message={verificationLabels[request.verification_type]||request.verification_type.replaceAll('_',' ')}/> · {new Date(request.submitted_at).toLocaleString()}{request.expires_on?` · expires ${request.expires_on}`:''}</div></div><StatusPill status={request.status}/>{request.review_note?<p>{request.review_note}</p>:null}<a href={`/api/files/verification/${request.id}`} target="_blank"><Text message="Open document"/></a></div>)}{!requestResult.items.length?<p className="muted"><Text message="No verification requests submitted yet."/></p>:null}</div><Pagination path="/app/verification" query={{subjectPage:query.subjectPage}} page={requestResult.page} pageCount={requestResult.pageCount} total={requestResult.total} pageParam="requestPage"/></aside>
    </div>
  </div>;
}
