...
export default async function FleetPage(){
 const user=await requireUser(); if(user.role!=='TRANSPORTER') redirect('/app/home');
 const data:any=getDashboard(user); const vehicles:any[]=listOwnVehicles(user); const capacities:any[]=listOwnCapacity(user); const latest=new Map(); for(const item of capacities)if(!latest.has(item.vehicle_id))latest.set(item.vehicle_id,item);
 return <div className="page"><PageHeader title="Fleet overview" subtitle="A company-wide view for dispatchers and fleet coworkers. Drivers update each truck from Home." action={<Link href="/app/home" className="button">Update a truck</Link>}/>
