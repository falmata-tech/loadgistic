...
export default async function CapacityPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const activeFilters=[
   ...
   filters.nearLat&&filters.nearLng?{label:`Near me · ${filters.nearRadiusKm} km`,href:clearFilterHref(['nearLat','nearLng','nearRadiusKm'])}:null
 ...
 <NearbyTruckSearch active={Boolean(filters.nearLat&&filters.nearLng)} radiusKm={Number(filters.nearRadiusKm)||10} trucks={rows.filter((cap:any)=>Number.isFinite(Number(cap.shared_location_lat))&&Number.isFinite(Number(cap.shared_location_lng))).map((cap:any)=>({id:cap.id,label:`${cap.vehicle_make} ${cap.vehicle_model}`,lat:Number(cap.shared_location_lat),lng:Number(cap.shared_location_lng),radiusKm:Number(cap.location_precision_km)||40}))}/>
 <BoardFilterSheet
 ...
 >
   ...
   <section className="board-filter-step"><div className="workflow-step-heading"><span>2</span><Truck aria-hidden="true"/><div><h3>Truck and space</h3><p>Choose the capacity you need.</p></div></div><div className="board-filter-grid">
     <div className="form-group"><label htmlFor="capacity-status"><Gauge aria-hidden="true"/>Availability</label><select id="capacity-status" name="status" defaultValue={filters.status}><option value="">Empty or Partial</option><option value="EMPTY">Empty</option><option value="PARTIAL">Partial</option></select></div>
   ...
   <section className="board-more-filters board-filter-step">
     ...
     <div className="board-filter-grid">
       ...
       <div className="form-group"><label htmlFor="capacity-stops"><Route aria-hidden="true"/>Extra stops</label><select id="capacity-stops" name="stopOption" defaultValue={filters.stopOption}><option value="">Any stop preference</option><option value="MULTI_PICK">Accepts Multi Pick</option><option value="MULTI_DROP">Accepts Multi Drop</option></select></div>
 ...
 <section className="capacity-market-grid">{rows.map((cap:any)=><article className="card capacity-market-card" key={cap.id}>
   ...
   <div className="market-signal-grid"><div><small>Current area</small><strong>{cap.possible_distance_min_km!=null?`Possibly ${cap.possible_distance_min_km}–${cap.possible_distance_max_km} km away`:cap.movement_scope!=='INTERCITY'&&cap.local_place_label?`${cap.local_place_label} · ${cap.local_radius_km} km radius`:cap.location_area||'Area not updated'}</strong><span>{cap.possible_distance_min_km!=null?`${cap.location_precision_km} km Driver privacy area · confirm directly`:cap.location_updated_at?`Location ${relativeTime(cap.location_updated_at)}`:'Location time unavailable'}</span></div><div><small>{cap.status==='PARTIAL'?'Live partial route':'Planned empty route'}</small>{cap.status==='PARTIAL'?<><strong>{cap.current_route_origin||'Not recorded'} → {cap.current_route_destination||'Not recorded'}</strong><span>Live now · refreshed {relativeTime(cap.updated_at)}</span></>:cap.origin&&cap.destination?<><strong>{cap.origin} → {cap.destination}</strong>{travelDay(cap.travel_date)?<span>{travelDay(cap.travel_date)}</span>:<span>Travel day not selected</span>}</>:<><strong>Willing to go anywhere</strong><span>{cap.movement_scope==='LOCAL'?'Within the selected Local area':'No specific route selected'}</span></>}</div><div><small>Accepting</small><strong>{acceptedLoads(cap)}</strong>{stopPolicy(cap)?<span>{stopPolicy(cap)}</span>:null}</div><div><small>Proof signal</small><strong>{cap.proof_available?'Photo recorded':'No photo'}</strong><span>{cap.proof_recorded_at?relativeTime(cap.proof_recorded_at):'Not recorded'}</span></div></div>
