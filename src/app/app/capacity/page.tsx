...
import { Boxes, CalendarClock, Camera, CircleDotDashed, Eye, Gauge, MapPin, Route, SlidersHorizontal, Truck } from 'lucide-react';
...
export default async function CapacityPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 >
   <div className="board-filter-grid">
     ...
     <div className="form-group"><label htmlFor="capacity-status"><Gauge aria-hidden="true"/>Cargo space</label><select id="capacity-status" name="status" defaultValue={filters.status}><option value="">Empty or Partial</option><option value="EMPTY">Empty</option><option value="PARTIAL">Partial</option></select></div>
 ...
 <section className="capacity-market-grid">{rows.map((cap:any)=><article className="card capacity-market-card" key={cap.id}>
   ...
   <div className="market-card-actions">{cap.route_match_label?<span className={`route-match match-${cap.route_match_score}`}><Route aria-hidden="true"/>{cap.route_match_label}{cap.route_match_source?` · ${cap.route_match_source}`:''}</span>:null}<Link className="button secondary small" href={`/app/capacity/${cap.id}`}><Eye aria-hidden="true"/>Truck details</Link></div>
