...
import { Boxes, CalendarClock, Camera, CircleDotDashed, Eye, Gauge, MapPin, Phone, Route, SlidersHorizontal, Truck } from 'lucide-react';
...
export default async function CapacityPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 <section className="capacity-market-grid">{rows.map((cap:any)=><article className="card capacity-market-card" key={cap.id}>
   ...
   {cap.status!=='BUSY'?<div className="progress"><span style={{width:`${cap.available_percent}%`}}/></div>:<div className="busy-board-notice"><CalendarClock aria-hidden="true"/><span><strong>Available again {cap.available_again_date}</strong><small>Busy now, but open to calls for future work.</small></span></div>}
   ...
   <div className="market-signal-grid"><div><small>{cap.status==='BUSY'?'Expected area':'Current area'}</small><strong>{cap.movement_scope!=='INTERCITY'&&cap.local_place_label?`${cap.local_place_label} · ${cap.local_radius_km} km radius`:cap.location_area||cap.origin||'Area not updated'}</strong><span>{cap.location_updated_at?`Location ${relativeTime(cap.location_updated_at)}`:'Location time unavailable'}</span></div>{cap.status==='BUSY'?<><div><small>Preferred Routes</small><strong>{cap.preferred_routes_label||'No routes declared'}</strong><span>Future-work preference, not current cargo space</span></div><div><small>Contact</small><strong>Calls welcome</strong><span>Capacity updated {relativeTime(cap.updated_at)}</span></div></>:<><div><small>{cap.movement_scope==='LOCAL'?'Movement':cap.status==='PARTIAL'?'Current partial route':'Planned route'}</small>{cap.movement_scope==='LOCAL'?<><strong>Local availability</strong><span>No between-city route required</span></>:<><strong>{cap.status==='PARTIAL'?(cap.current_route_origin||'Not recorded'):(cap.origin||'Not recorded')} → {cap.status==='PARTIAL'?(cap.current_route_destination||'Not recorded'):(cap.destination||'Not recorded')}</strong><span>{cap.status==='PARTIAL'?(cap.current_route_date||'No current-route date'):(cap.travel_date?`${cap.travel_date} · ${cap.planned_space_status==='PARTIAL'?'Partial':'Full'} cargo space`:'No planned date')}</span></>}</div><div><small>Accepting</small><strong>{acceptedLoads(cap)}</strong><span>{stopPolicy(cap)} · {cap.open_to_contract_lanes?'Contract routes':'Single-trip work'}</span></div><div><small>Proof signal</small><strong>{cap.proof_available?'Photo recorded':'No photo'}</strong><span>{cap.proof_recorded_at?relativeTime(cap.proof_recorded_at):'Not recorded'}</span></div></>}</div>
   <div className="market-card-actions">{cap.route_match_label?<span className={`route-match match-${cap.route_match_score}`}><Route aria-hidden="true"/>{cap.route_match_label}{cap.route_match_source?` · ${cap.route_match_source}`:''}</span>:null}{!isProvider&&cap.provider_contact_phone?<a className="button secondary small icon-button-label" href={`tel:${cap.provider_contact_phone}`}><Phone aria-hidden="true"/>Call</a>:null}<Link className="button secondary small" href={`/app/capacity/${cap.id}`}><Eye aria-hidden="true"/>Truck details</Link></div>
