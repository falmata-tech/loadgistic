  ...
  return <div className="page">
    ...
    <div className="two-col">
      <div className="stack">
        ...
        {capacity.status!=='BUSY'?<section className="card"><h2 className="panel-heading"><Gauge aria-hidden="true"/>Accepted work</h2><div className="detail-facts"><div><span>Load size</span><strong>{acceptedLoads(capacity)}</strong></div><div><span>Stops</span><strong>{stopPolicy(capacity)}</strong></div><div><span>Contract routes</span><strong>{capacity.open_to_contract_lanes ? 'Recurring routes' : 'Single trip'}</strong></div><div><span>Photo proof</span><strong>{capacity.proof_available ? 'Recorded' : 'Not added'}</strong></div></div></section>:null}
      </div>
