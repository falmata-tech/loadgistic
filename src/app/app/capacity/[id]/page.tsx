    ...
    <div className="two-col">
      <div className="stack">
        <section className="card capacity-detail-hero">
          ...
          <div><span className="meta">{capacity.status==='BUSY'?'Future availability':'Current cargo space'}</span><h2>{capacityLabel(capacity.status,capacity.available_percent)}</h2>{capacity.status!=='BUSY'?<div className="progress"><span style={{width:`${capacity.available_percent}%`}}/></div>:<p><strong>Available again {capacity.available_again_date}</strong></p>}<p className="muted">This capacity signal belongs to this specific truck, not to the transporter&apos;s fleet as a whole.</p></div>
        </section>
        <section className="card"><h2 className="panel-heading"><Route aria-hidden="true"/>Movement</h2><div className="detail-facts"><div><span>{capacity.status==='BUSY'?'Expected area':'Current area'}</span><strong>{capacity.location_area || 'Area not updated'}</strong><small>{capacity.location_source === 'DEVICE_OBSCURED' ? `Approximate device area · ${capacity.location_precision_km} km privacy zone` : 'Driver-declared general area'}</small></div>{capacity.status==='PARTIAL'?<div><span>Live partial route</span><strong>{capacity.current_route_origin||'Not recorded'} → {capacity.current_route_destination||'Not recorded'}</strong><small>Live now · refreshed {relativeTime(capacity.updated_at)}</small></div>:null}{capacity.status!=='BUSY'?<><div><span>Planned route</span><strong>{capacity.origin || 'Not recorded'} → {capacity.destination || 'Not recorded'}</strong><small>{capacity.travel_date ? `${capacity.travel_date} · ${capacity.planned_space_status==='PARTIAL'?'Partial':'Full'} space` : 'Not scheduled'}</small></div><div><span>Next available</span><strong>{capacity.next_available || 'Confirm directly'}</strong></div></>:null}</div></section>
        {capacity.status!=='BUSY'?<section className="card"><h2 className="panel-heading"><Gauge aria-hidden="true"/>Accepted work</h2><div className="detail-facts"><div><span>Shipment size</span><strong>{acceptedLoads(capacity)}</strong></div><div><span>Stops</span><strong>{stopPolicy(capacity)}</strong></div><div><span>Contract routes</span><strong>{capacity.open_to_contract_lanes ? 'Recurring routes' : 'Single trip'}</strong></div><div><span>Photo proof</span><strong>{capacity.proof_available ? 'Recorded' : 'Not added'}</strong></div></div></section>:null}
      </div>
      <aside className="stack">
        ...
        <section className="card"><h3><Clock3 aria-hidden="true"/>Freshness</h3><StatusPill status={capacity.freshness}/><p className="meta">Capacity {relativeTime(capacity.updated_at)}<br/>Location {capacity.location_updated_at ? relativeTime(capacity.location_updated_at) : 'not recorded'}{capacity.status==='BUSY'?<><br/>Available again {capacity.available_again_date}</>:null}</p>{capacity.freshness==='UPDATE_NEEDED'?<p className="alert warning">Old update. Confirm directly.</p>:null}</section>
