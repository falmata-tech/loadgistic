    ...
    <div className="two-col">
      <div className="stack">
        <section className="card capacity-detail-hero">
          ...
          <div><span className="meta">{capacity.status==='BUSY'?'Future availability':'Current cargo space'}</span><h2>{capacityLabel(capacity.status,capacity.available_percent)}</h2>{capacity.status!=='BUSY'?<div className="progress"><span style={{width:`${capacity.available_percent}%`}}/></div>:<p><strong>Available {capacity.available_again_date}{capacity.available_again_place_label?` near ${capacity.available_again_place_label}`:''}</strong></p>}<p className="muted">This capacity signal belongs to this specific truck, not to the transporter&apos;s fleet as a whole.</p></div>
        </section>
        <section className="card"><h2 className="panel-heading"><Route aria-hidden="true"/>Movement</h2><div className="detail-facts"><div><span>Current area</span><strong>{capacity.location_area || 'Area not updated'}</strong><small>{capacity.location_source === 'DEVICE_OBSCURED' ? `Approximate Driver device area · ${capacity.location_precision_km} km privacy zone` : 'Location unavailable'}</small></div>{capacity.status==='BUSY'?<div><span>Available near</span><strong>{capacity.available_again_place_label||'Expected area not recorded'}</strong><small>{capacity.available_again_date?`Ready ${capacity.available_again_date}`:'Ready date unavailable'}</small></div>:null}{capacity.status==='PARTIAL'?<div><span>Live partial route</span><strong>{capacity.current_route_origin||'Not recorded'} → {capacity.current_route_destination||'Not recorded'}</strong><small>Live now · refreshed {relativeTime(capacity.updated_at)}</small></div>:null}{capacity.status==='EMPTY'?<div><span>Route plan</span><strong>{capacity.origin&&capacity.destination?`${capacity.origin} → ${capacity.destination}`:'Willing to go anywhere'}</strong><small>{capacity.travel_date?`Travel ${capacity.travel_date}`:'No specific route selected'}</small></div>:null}</div></section>
