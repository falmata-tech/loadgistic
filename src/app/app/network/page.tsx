...
import { getNetworkState, listNetwork } from '@/lib/repository.js';
...
export default async function NetworkPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <div className="page"><PageHeader title="My Network" subtitle="Keep private Favorites separate from mutual transport partnerships." action={<Link className="button icon-button-label" href="/app/providers"><Network aria-hidden="true"/>Find members</Link>}/><Flash error={query.error} success={query.success}/>
    ...
    <div className="network-list">{rows.map(row=>{const state=getNetworkState(user,row.target_kind,row.target_id);return <article className="card network-row" key={row.id}><div className="company-logo small">{row.name.split(' ').slice(0,2).map((word:string)=>word[0]).join('')}</div><div><h3><Link href={`/companies/${row.handle}`}>{row.name}</Link></h3><p className="meta">{row.city||'Location not added'}</p>{row.incoming?<span className="status">Incoming request</span>:row.outgoing?<span className="status">Request sent</span>:row.status==='DECLINED'?<span className="status expired">Not connected</span>:null}</div><NetworkActions state={state} targetKind={row.target_kind} targetId={row.target_id} returnTo={`/app/network?view=${view}`}/></article>})}</div>
    {!rows.length?<div className="empty-state"><selected.icon aria-hidden="true"/><strong>No {selected.label.toLowerCase()} yet.</strong><span>Use the authenticated Directory to find Businesses and transport providers.</span></div>:null}
  </div>;
}
