  ...
  return <div className="page">
    ...
    <div className="stack">
      {memberResult.items.map((load:any)=><article className="card pooled-member" key={load.id}>
        <div><div className="status-row"><span className="status green">Partial Truckload (PTL)</span><span className="status">{load.shipper_name}</span>{load.interested?<span className="status green">Interest sent</span>:null}</div><h3>{load.title}</h3><div className="route">{load.origin}<span>→</span>{load.destination}</div><p>{load.cargo_description}</p><div className="meta">Pick up before {load.pickup_date}{load.delivery_date?` · Drop off before ${load.delivery_date}`:''}</div></div>
