...
import { ArrowRight, Banknote, Boxes, Building2, CalendarClock, Check, CircleDotDashed, Clock3, Eye, Layers3, MapPinned, Phone, Route, Send, SlidersHorizontal, Truck } from 'lucide-react';
  ...
  return <div className="page">
    ...
    {!canBrowse?<div className="permission-note"><SlidersHorizontal aria-hidden="true"/><div><strong>Shipment Board access is managed by your fleet owner</strong><span>You can continue using duty and assigned tracking workflows.</span></div></div>:<>
      ...
      >
        ...
        {board==='LOADS'?<section className="board-filter-step"><div className="workflow-step-heading"><span>1</span><Route aria-hidden="true"/><div><h3>Route or area</h3><p>Start with where the shipment moves.</p></div></div><div className="board-filter-grid">
        ...
        {board==='LOADS'?<section className="board-filter-step"><div className="workflow-step-heading"><span>2</span><Truck aria-hidden="true"/><div><h3>Shipment and truck</h3><p>Choose the work that fits.</p></div></div><div className="board-filter-grid"><div className="form-group"><label htmlFor="load-type"><Boxes aria-hidden="true"/>Shipment size</label><select id="load-type" name="loadType" defaultValue={filters.loadType}><option value="">Any shipment size</option><option value="FTL">Full Truckload (FTL)</option><option value="PTL">Partial Truckload (PTL)</option></select></div>
