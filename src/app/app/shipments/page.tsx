...
export default async function ShipmentsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="page">
    <PageHeader icon={view==='MY_LOADS'?ClipboardList:MapPin} title={isBusiness?'My Loads':'Tracking'} subtitle={view==='MY_LOADS'?'Posted and active loads.':'Loads involving you.'} action={isBusiness?<Link href="/app/shipments/new" className="button icon-button-label"><CirclePlus aria-hidden="true"/>Post load</Link>:undefined}/>
    <Flash error={query.error} success={query.success}/>
    {overdueLoads.length?<div className="permission-note warning"><ClockAlert aria-hidden="true"/><div><strong>{overdueLoads.length} {overdueLoads.length===1?'load needs':'loads need'} review</strong><span>{hiddenLoadCount?`${hiddenLoadCount} ${hiddenLoadCount===1?'is':'are'} now off the Load Board. `:''}Past-due requests stay on the Board for two full grace days, then remain only in My Loads.</span></div></div>:null}
