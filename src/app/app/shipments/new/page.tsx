...
export default async function NewShipmentPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 <form action="/api/shipments" method="post" className="form-card stack" data-testid="new-shipment-form">
  <div className="form-grid">
   ...
   <div className="form-group"><label htmlFor="providerRef">Selected provider</label><select id="providerRef" name="providerRef" defaultValue={query.provider || ''}><option value="">Choose for a direct request</option>{providers.map((p:any)=><option key={`${p.ref_kind}-${p.id}`} value={`${p.ref_kind}:${p.id}`}>{p.name} — {p.type.replaceAll('_',' ')}</option>)}</select></div>
