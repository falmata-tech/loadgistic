...
export default async function NewShipmentPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <div className="page"><PageHeader title="New freight load" subtitle="Post boxes, pallets, partial truckloads, or full truckloads for approved transporters."/><Flash error={query.error}/>
 <ShipmentForm workspaceName={user.organization_name||user.name} businesses={businessOptions} providers={providerOptions} selectedProvider={query.provider||''} minDate={today}/></div>;
}
