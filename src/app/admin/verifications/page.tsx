...
export default async function AdminVerificationsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  const requests:any[]=listVerificationRequests(user);
  return <div className="page"><PageHeader title="Verification requests" subtitle="Review private identity, license, driver, and truck evidence."/><Flash error={query.error} success={query.success}/>
    <div className="stack">{requests.map(request=><article className="card verification-review-card" key={request.id}>
    ...
    </article>)}{!requests.length?<div className="empty-state">No verification requests.</div>:null}</div>
  </div>;
}
