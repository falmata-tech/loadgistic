  ...
  return <div className="page admin-operations">
    ...
    <section className="admin-record-section">
      <div className="admin-record-list">
        ...
        {view==='CAPACITY'?records.map(record=><article key={record.id}><div><strong>{record.platform_number} · {record.make} {record.model}</strong><span>{record.owner_name} · {record.location_area||'Area not updated'}</span><small>{record.status==='PARTIAL'?'Partial space':'Empty truck'} · {record.visibility.replaceAll('_',' ')} · updated {relativeTime(record.updated_at)}</small></div><StatusPill status={record.status}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link>{record.status!=='OFF_DUTY'?<form action={`/api/admin/records/capacity/${record.id}`} method="post"><input type="hidden" name="command" value="OFF_DUTY"/><input type="hidden" name="returnTo" value={returnTo}/><button className="button small danger"><Gauge aria-hidden="true"/>Off Duty</button></form>:null}</article>):null}
        {view==='ROUTES'?records.map(record=><article key={`${record.record_kind}-${record.id}`}><div><strong>{record.owner_name}</strong><span>{record.record_kind==='SERVICE_AREA'?`${record.origin} · ${record.radius_km} km radius`:`${record.origin} → ${record.destination}`}</span><small>{record.record_kind==='SERVICE_AREA'?'Local service area':'Preferred Route'} · added {relativeTime(record.created_at)}</small></div><StatusPill status={record.record_kind==='SERVICE_AREA'?'LOCAL':'ROUTE'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><form action={`/api/admin/records/${record.record_kind.toLowerCase()}/${record.id}`} method="post"><input type="hidden" name="command" value="REMOVE"/><input type="hidden" name="returnTo" value={returnTo}/><button className="button small danger"><Trash2 aria-hidden="true"/>Remove</button></form></article>):null}
