...
export default async function AdminOperationsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <div className="page admin-operations">
    <PageHeader title="Platform Records" subtitle="Find and manage one record type at a time."/>
    ...
    <section className="stats compact-admin-stats">
      ...
      <div className="stat"><span className="meta">Current capacity</span><strong>{data.counts.board_capacity}</strong></div>
    </section>
    ...
    <section className="admin-record-section">
      <div className="admin-record-list">
        {view==='USERS'?records.map(record=><article key={record.id}><div><strong>{record.name}</strong><span>{record.email} · {record.phone||'Phone not added'}</span><small>{record.workspace_name} · {record.role.replaceAll('_',' ')}</small></div><StatusPill status={record.active?'ACTIVE':'SUSPENDED'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><form action={`/api/admin/records/user/${record.id}`} method="post"><input type="hidden" name="returnTo" value={returnTo}/>{!record.active?<input type="hidden" name="active" value="on"/>:null}<button className={`button small icon-button-label ${record.active?'danger':'success'}`} disabled={record.id===user.id}>{record.active?'Suspend':'Restore'}</button></form></article>):null}
        {view==='WORKSPACES'?records.map(record=><article key={`${record.record_kind}-${record.id}`}><div><strong>{record.name}</strong><span>{workspaceTypeLabel(record.type)} · {record.city||'Location not added'}</span><small>{record.user_count} users · {record.truck_count} active trucks · {record.tracking_count} Tracking sessions</small></div><StatusPill status={record.subscription_status||'NO PLAN'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link></article>):null}
        {view==='TRUCKS'?records.map(record=><article key={record.id}><div><strong>{record.platform_number} · {record.make} {record.model}</strong><span>{record.owner_name} · {record.cargo_configuration} · plate {record.plate||'not recorded'}</span><small>{record.capacity_status?`${record.capacity_status.replaceAll('_',' ')} · ${record.location_area||'Area not updated'} · ${relativeTime(record.capacity_updated_at)}`:'No capacity update'}</small></div><StatusPill status={record.active?'ACTIVE':'INACTIVE'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><form action={`/api/admin/records/vehicle/${record.id}`} method="post"><input type="hidden" name="returnTo" value={returnTo}/>{!record.active?<input type="hidden" name="active" value="on"/>:null}<button className={`button small icon-button-label ${record.active?'danger':'success'}`}>{record.active?'Deactivate':'Reactivate'}</button></form></article>):null}
        {view==='DRIVERS'?records.map(record=><article key={record.id}><div><strong>{record.name}</strong><span>{record.owner_name} · {record.platform_number||'No assigned truck'}</span><small>{[record.can_manage_capacity?'Capacity updates':null,record.can_manage_tracking?'Tracking updates':null].filter(Boolean).join(' · ')||'Duty control only'}</small></div><StatusPill status={record.active?'ACTIVE':'SUSPENDED'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><details className="admin-inline-actions"><summary className="button small secondary">Permissions</summary><form action={`/api/admin/records/driver_permissions/${record.id}`} method="post" className="team-permission-grid"><input type="hidden" name="returnTo" value={returnTo}/><label className="checkbox-control"><input name="canManageCapacity" type="checkbox" defaultChecked={Boolean(record.can_manage_capacity)}/>Capacity updates</label><label className="checkbox-control"><input name="canManageTracking" type="checkbox" defaultChecked={Boolean(record.can_manage_tracking)}/>Tracking updates</label><button className="button small">Save</button></form></details><form action={`/api/admin/records/driver/${record.id}`} method="post">{!record.active?<input type="hidden" name="active" value="on"/>:null}<input type="hidden" name="returnTo" value={returnTo}/><button className={`button small ${record.active?'danger':'success'}`}>{record.active?'Suspend':'Restore'}</button></form></article>):null}
        {view==='TRACKING'?records.map(record=><article key={record.id}><div><strong>{record.code} · {record.cargo_summary}</strong><span>{record.origin} → {record.destination}</span><small>{record.provider_name} · {record.platform_number} · Driver {record.driver_name} · updated {relativeTime(record.updated_at)}</small></div><StatusPill status={record.operational_status}/><Link className="button secondary small" href={recordHref(view,record)}><PackageSearch aria-hidden="true"/>Open Tracking</Link>{record.provider_handle?<Link className="button secondary small" href={`/@${record.provider_handle}`}><Building2 aria-hidden="true"/>Transporter</Link>:null}</article>):null}
        {view==='CAPACITY'?records.map(record=><article key={record.id}><div><strong>{record.platform_number} · {record.make} {record.model}</strong><span>{record.owner_name} · {record.location_area||'Area not updated'}</span><small>{record.status==='PARTIAL'?'Partial space':record.status==='EMPTY'?'Empty truck':record.status==='OFF_DUTY'?'Off Duty':record.status.replaceAll('_',' ')} · {record.visibility.replaceAll('_',' ')} · updated {relativeTime(record.updated_at)}</small></div><StatusPill status={record.status}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link>{record.status!=='OFF_DUTY'?<form action={`/api/admin/records/capacity/${record.id}`} method="post"><input type="hidden" name="command" value="OFF_DUTY"/><input type="hidden" name="returnTo" value={returnTo}/><button className="button small danger"><Gauge aria-hidden="true"/>Off Duty</button></form>:null}</article>):null}
        {view==='ROUTES'?records.map(record=><article key={`${record.record_kind}-${record.id}`}><div><strong>{record.owner_name}</strong><span>{record.geometry==='RADIUS'?`Area around ${record.origin}`:`${record.origin} → ${record.destination}`}</span><small>{record.geometry==='RADIUS'?'Service area':'Regular route'} · added {relativeTime(record.created_at)}</small></div><StatusPill status={record.geometry==='RADIUS'?'SERVICE_AREA':'ROUTE'}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><form action={`/api/admin/records/${record.record_kind.toLowerCase()}/${record.id}`} method="post"><input type="hidden" name="command" value="REMOVE"/><input type="hidden" name="returnTo" value={returnTo}/><button className="button small danger"><Trash2 aria-hidden="true"/>Remove</button></form></article>):null}
        {view==='SUBSCRIPTIONS'?records.map(record=><article key={record.id}><div><strong>{record.owner_name}</strong><span>{record.plan_name} · {record.billing_model.replaceAll('_',' ')}</span><small>{record.ends_at?`Ends ${new Date(record.ends_at).toLocaleDateString()}`:'No expiry'} · updated {relativeTime(record.updated_at)}</small></div><StatusPill status={record.status}/><Link className="button secondary small" href={recordHref(view,record)}><Eye aria-hidden="true"/>Open</Link><details className="admin-inline-actions"><summary className="button small secondary">Manage</summary><form action={`/api/admin/records/subscription/${record.id}`} method="post" className="button-row"><input type="hidden" name="returnTo" value={returnTo}/><button className="button small success" name="command" value="PAID">Paid · 30 days</button><button className="button small danger" name="command" value="EXPIRE">Expire</button></form></details></article>):null}
      </div>
