...
function RecordActions({record,returnTo,actorId}:{record:any;returnTo:string;actorId:string}){
  if(record.view==='WORKSPACES')return <form className="stack" action={`/api/admin/workspaces/${record.id}/details`} method="post"><input type="hidden" name="kind" value={record.record_kind}/><input type="hidden" name="previous_name" value={record.name}/><label><Text message="Business name"/><input name="name" defaultValue={record.name} minLength={2} maxLength={120} required/></label><label><Text message="Reason"/><textarea name="reason" minLength={5} maxLength={500} required/></label><button className="button secondary"><Text message="Save business name"/></button></form>;
  if(record.view==='USERS')return <form action={`/api/admin/records/user/${record.id}`} method="post"><input type="hidden" name="returnTo" value={returnTo}/>{!record.active?<input type="hidden" name="active" value="on"/>:null}<button className={`button ${record.active?'danger':'success'}`} disabled={record.id===actorId} title={record.id===actorId?'Administrators cannot suspend their own account.':undefined}>{record.id===actorId?<Text message="Current account"/>:record.active?<Text message="Suspend account"/>:<Text message="Restore account"/>}</button></form>;
  if(record.view==='TRUCKS')return <form action={`/api/admin/records/vehicle/${record.id}`} method="post"><input type="hidden" name="returnTo" value={returnTo}/>{!record.active?<input type="hidden" name="active" value="on"/>:null}<button className={`button ${record.active?'danger':'success'}`}>{record.active?<Text message="Deactivate truck"/>:<Text message="Reactivate truck"/>}</button></form>;
...
export default async function AdminOperationRecordPage({params,searchParams}:{params:Promise<{view:string;id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="page admin-operation-record-page">
    ...
    <section className="admin-operation-record-card">
      <header><div><span className="section-kicker">{config.label}</span><h2><Text message="Record details"/></h2></div><StatusPill status={statusFor(record)}/></header>
      <dl>{factsFor(record).map(([label,value])=><div key={label}><dt>{label}</dt><dd>{text(value)}</dd></div>)}</dl>
      <div className="admin-record-links">
        {record.provider_handle?<Link className="button secondary" href={`/@${record.provider_handle}`}><ExternalLink aria-hidden="true"/><Text message="Public transporter"/></Link>:null}
        {record.vehicle_id&&view!=='TRUCKS'?<Link className="button secondary" href={`/admin/operations/trucks/${record.vehicle_id}`}><Truck aria-hidden="true"/><Text message="Open truck"/></Link>:null}
        {record.workspace_id?<Link className="button secondary" href={`/admin/operations/workspaces/${record.workspace_id}?kind=${record.workspace_kind}`}><Building2 aria-hidden="true"/><Text message="Open workspace"/></Link>:null}
        {record.owner_id?<Link className="button secondary" href={`/admin/operations/workspaces/${record.owner_id}?kind=${record.owner_kind}`}><Building2 aria-hidden="true"/><Text message="Open owner"/></Link>:null}
      </div>
    </section>
