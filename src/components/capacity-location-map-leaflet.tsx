...
export function CapacityLocationMapLeaflet({center,radiusKm,areaCenter=null,areaBoundary=[],routePoints=[],regularSignals=[],availabilityStatus='EMPTY',showLegend=true,showTruckLabel=true}:{center:Point;radiusKm:number;areaCenter?:Point|null;areaBoundary?:PlacePoint[];routePoints?:PlacePoint[];regularSignals?:RegularSignal[];availabilityStatus?:string;showLegend?:boolean;showTruckLabel?:boolean}){
  ...
  return <div className="capacity-location-map" aria-label={`${radiusKm} kilometre approximate truck location`}>
    <MapContainer center={[center.lat,center.lng]} zoom={8} minZoom={5} maxZoom={13} maxBounds={EAST_AFRICA_MAP_BOUNDS} maxBoundsViscosity={0.85} scrollWheelZoom={false} dragging>
      ...
      <Circle center={[center.lat,center.lng]} radius={radiusKm*1000} pathOptions={{color:'#1a73e8',weight:7,fill:false,dashArray:'7 7'}}><Tooltip className="capacity-route-tooltip location"><Text message="Approximate truck location"/><br/><Text message="Within "/>{radiusKm}<Text message=" km · not an exact position"/></Tooltip></Circle>
      {areaBoundary.length>=3?<Polygon positions={areaBoundary.map(point=>[point.lat,point.lng])} pathOptions={{color:capacityColor,weight:8,fill:false}}><Tooltip className={currentTooltipClass}>{capacityLabel}<Text message=" Service area"/><br/>{areaBoundary.map(point=>point.label).join(' · ')}<br/><Text message="Confirm current availability and cargo fit"/></Tooltip></Polygon>:null}
      {areaCenter?<CircleMarker center={[areaCenter.lat,areaCenter.lng]} radius={5} pathOptions={{color:capacityColor,fillColor:'#fff',fillOpacity:1,weight:2}}><Tooltip className={currentTooltipClass}><Text message="Service area center"/></Tooltip></CircleMarker>:null}
      {routePoints.length>=2?<Polyline positions={routePoints.map(point=>[point.lat,point.lng])} pathOptions={{color:capacityColor,weight:8,opacity:.95}}><Tooltip className={currentTooltipClass}>{capacityLabel}<Text message=" route"/><br/>{routePoints.map(point=>point.label).join(' → ')}<br/><Text message="Confirm current availability and cargo fit"/></Tooltip></Polyline>:null}
      {regularSignals.map(signal=>signal.geometry==='RADIUS'?<Polygon key={signal.id} positions={signal.area_boundary.map(point=>[point.lat,point.lng])} pathOptions={{color:'#c06620',weight:7,dashArray:'6 7',fill:false}}><Tooltip className="capacity-route-tooltip regular">{signal.area_center_label||'Regular Service area'}<br/><Text message="Regular service · confirm current availability"/></Tooltip></Polygon>:<Polyline key={signal.id} positions={signal.route_points.map(point=>[point.lat,point.lng])} pathOptions={{color:'#c06620',weight:7,dashArray:'5 8',opacity:.9}}><Tooltip className="capacity-route-tooltip regular">{signal.route_points.map(point=>point.label).join(' ↔ ')}<br/><Text message="Regular capacity route · confirm current availability"/></Tooltip></Polyline>)}
      ...
      <CircleMarker center={[center.lat,center.lng]} radius={9} pathOptions={{className:'capacity-setting-truck-marker',color:'#fff',weight:4,fillColor:'#1a73e8',fillOpacity:1}}>{showTruckLabel?<Tooltip className="capacity-setting-truck-label" permanent direction="top" offset={[0,-9]}><Text message="Truck area"/></Tooltip>:<Tooltip direction="top" offset={[0,-9]}><Text message="Approximate truck location"/></Tooltip>}</CircleMarker>
    </MapContainer>
    {showLegend?<div className="capacity-map-key"><span><i className={`truck ${availabilityStatus==='PARTIAL'?'partial-truck':'empty-truck'}`}/>{availabilityStatus==='PARTIAL'?<Text message="Partial"/>:<Text message="Empty"/>}<Text message=" truck marker"/></span><span><i/><Text message="Approximate location radius"/></span>{areaBoundary.length>=3?<span><i className={availabilityStatus==='PARTIAL'?'partial-area':'empty-area'}/>{capacityLabel}<Text message=" area"/></span>:null}{routePoints.length>=2?<span><i className={availabilityStatus==='PARTIAL'?'partial-route':'empty-route'}/>{capacityLabel}<Text message=" route"/></span>:null}{regularSignals.length?<span><i className="corridor"/><Text message="Regular service"/></span>:null}</div>:null}
  </div>;
}
