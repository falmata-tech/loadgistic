...
type CapacitySnapshot = {
  ...
  current_route_origin?:string; current_route_destination?:string; current_route_date?:string;
    ...
    {onDuty?<div className="capacity-primary-flow" key={vehicleId}>
      ...
      <section className="control-panel capacity-decision">
        ...
        <div className="segmented-control movement-scope-control visual-scope-control" role="group" aria-label="Work area">
          ...
          <button type="button" aria-pressed={movementScope==='INTERCITY'} disabled={!interactive} onClick={()=>chooseMovementScope('INTERCITY')}><Route aria-hidden="true"/>Between cities</button>
      ...
      <details className="control-panel capacity-disclosure capacity-options">
        <summary><div className="panel-title-copy"><ChevronDown aria-hidden="true"/><div><h2>Optional details</h2><p>Preferences, future trip, sharing, photo.</p></div></div><span>{visibility==='OPEN'?'Public':'Partners'}</span></summary>
        <div className="capacity-disclosure-body">
          {status!=='BUSY'?<section className="capacity-option-group"><h3><Boxes aria-hidden="true"/>Shipment preferences</h3><div className="segmented-control load-policy-control">{['FTL','PTL','BOTH'].map(value=><label key={value}><input name="acceptedLoads" value={value} type="radio" checked={value===acceptedLoads} onChange={()=>setAcceptedLoads(value)}/><span>{value==='BOTH'?'Both':value}</span></label>)}</div><div className="multi-stop-toggles"><label className="rich-toggle"><input name="acceptsMultiPick" type="checkbox" defaultChecked={Boolean(current?.accepts_multi_pick)}/><span className="toggle-track" aria-hidden="true"/><span><strong>Multi Pick</strong><small>More than one pickup</small></span></label><label className="rich-toggle"><input name="acceptsMultiDrop" type="checkbox" defaultChecked={Boolean(current?.accepts_multi_drop)}/><span className="toggle-track" aria-hidden="true"/><span><strong>Multi Drop</strong><small>More than one drop-off</small></span></label></div><label className="rich-toggle"><input name="openToContractLanes" type="checkbox" defaultChecked={Boolean(current?.open_to_contract_lanes)}/><span className="toggle-track" aria-hidden="true"/><span><strong>Contract routes</strong><small>Open to recurring work</small></span></label></section>:null}
          {status!=='BUSY'&&movementScope!=='LOCAL'?<section className="capacity-option-group"><h3><Route aria-hidden="true"/>Future trip</h3><div className="route-inputs"><div className="form-group"><label htmlFor="capacity-origin"><MapPin aria-hidden="true"/>From</label><EthiopiaPlaceInput id="capacity-origin" name="origin" placeRefName="originPlaceRef" defaultPlaceRef={current?.origin_place_ref||''} defaultValue={current?.origin||''} placeholder="Addis Ababa, Ethiopia"/></div><div className="route-arrow" aria-hidden="true">→</div><div className="form-group"><label htmlFor="capacity-destination"><MapPin aria-hidden="true"/>To</label><EthiopiaPlaceInput id="capacity-destination" name="destination" placeRefName="destinationPlaceRef" defaultPlaceRef={current?.destination_place_ref||''} defaultValue={current?.destination||''} placeholder="Dire Dawa, Ethiopia"/></div></div><div className="form-grid"><div className="form-group"><label htmlFor="capacity-travel-date"><CalendarClock aria-hidden="true"/>Travel date</label><input id="capacity-travel-date" name="travelDate" type="date" defaultValue={current?.travel_date||''}/></div><div className="form-group"><label htmlFor="capacity-next"><Clock3 aria-hidden="true"/>Ready time</label><input id="capacity-next" name="nextAvailable" defaultValue={current?.next_available||''} placeholder="Tomorrow morning"/></div></div><div className="segmented-control"><label><input name="plannedSpaceStatus" value="FULL" type="radio" defaultChecked={(current?.planned_space_status||'FULL')==='FULL'}/><span>Full</span></label><label><input name="plannedSpaceStatus" value="PARTIAL" type="radio" defaultChecked={current?.planned_space_status==='PARTIAL'}/><span>Partial</span></label></div></section>:null}
