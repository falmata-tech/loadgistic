...
export function CapacityForm({vehicles,initialVehicleId,allowDeviceLocation=true,lockVehicleSelection=false}:{vehicles:VehicleOption[];initialVehicleId?:string;allowDeviceLocation?:boolean;lockVehicleSelection?:boolean}){
  ...
  const locationMessage=locationState==='captured'?'Truck area ready':locationState==='requesting'?'Finding truck location...':locationState==='denied'?'Location permission is off':locationState==='error'?'Device location unavailable':'Waiting for location';
