...
export function ProfileServiceAreaFields({initialAreas=[]}:{initialAreas?:ServiceArea[]}) {
  ...
  return <section className="profile-coverage-editor">
    <div className="section-heading-icon">
      ...
      <div><h2><Text message="Local Service Areas"/></h2><p className="meta"><Text message="Add each city or town separately and choose how far around it you normally work."/></p></div>
    </div>
    <div className="service-area-editor-list">
      {areas.map((area:EditableServiceArea,index:number)=><div className="service-area-editor-row" key={area.key}>
        <div className="form-group">
          <label htmlFor={`service-area-${index}`}><MapPin aria-hidden="true"/><Text message="City or town"/></label>
        ...
        <div className="form-group">
          <label htmlFor={`service-radius-${index}`}><CircleDotDashed aria-hidden="true"/><Text message="Operating radius"/></label>
          <select id={`service-radius-${index}`} name="serviceAreaRadiusKm" defaultValue={String(area.radius_km||25)} required>
            {[10,25,40,60,100].map(radius=><option value={radius} key={radius}>{radius}<Text message=" km"/></option>)}
          </select>
        </div>
        <Localized as="button" copy={["title","aria-label"]} className="icon-button danger" type="button" title="Remove service area" aria-label="Remove service area" onClick={()=>setAreas(current=>current.filter((item:EditableServiceArea)=>item.key!==area.key))}><Trash2 aria-hidden="true"/></Localized>
      </div>)}
    </div>
    {areas.length<8?<button className="button secondary icon-button-label" type="button" onClick={()=>setAreas(current=>[...current,{key:crypto.randomUUID(),radius_km:25}])}><Plus aria-hidden="true"/><Text message="Add Local Service Area"/></button>:null}
  </section>;
}
