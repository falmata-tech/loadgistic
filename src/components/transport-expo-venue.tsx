...
function ProviderIdentity({provider}:{provider:any}){
  return <Localized as="img" copy={["alt"]} src={provider.profile_image_url||'/marketing/default-transporter-profile.png'} alt=""/>;
}
...
function DriverPortrait({truck}:{truck:any}){
  ...
  if(truck.driver_portrait_url&&truck.driver_portrait_url!==failedUrl)return <Localized as="img" copy={["alt"]} src={truck.driver_portrait_url} alt="" onError={()=>setFailedUrl(truck.driver_portrait_url)}/>;
...
function TruckPreview({truck,walkthrough,onClose}:{truck:any;walkthrough:any;onClose:()=>void}){
  ...
  return <dialog ref={dialogRef} className="expo-provider-dialog featured-truck-dialog" aria-labelledby="featured-truck-dialog-title" onCancel={event=>{event.preventDefault();onClose();}} onClick={event=>{if(event.target===dialogRef.current)onClose();}}>
    <article>
      <Localized as="button" copy={["aria-label"]} type="button" onClick={onClose} aria-label="Close featured truck details"><X aria-hidden="true"/></Localized>
      ...
      <div className="featured-truck-dialog-time"><Clock3 aria-hidden="true"/><span><small><Text message="Programme time"/></small><strong>{walkthrough?.label||'07:30–09:00 EAT'}</strong></span></div>
      <div className="expo-dialog-actions"><Link className="button" href={`/@${encodeURIComponent(truck.handle)}`}><Building2 aria-hidden="true"/><Text message="Transporter profile"/></Link>{truck.public_capacity_available?<Link className="button secondary" href={`/?q=${encodeURIComponent(truck.platform_number||truck.cargo_configuration)}`}><Truck aria-hidden="true"/><Text message="Find this truck"/></Link>:null}</div>
    </article>
  </dialog>;
}
...
function SponsoredProviders({providers}:{providers:any[]}){
  ...
  return <aside className="expo-sponsored-rail" aria-labelledby="sponsored-provider-title" onFocusCapture={()=>{focusWithin.current=true;}} onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node|null))focusWithin.current=false;}}>
    <header><span><BadgeDollarSign aria-hidden="true"/><Text message="Sponsored"/></span><h3 id="sponsored-provider-title"><Text message="Sponsors"/></h3></header>
    <div aria-live="off">{visible.map(provider=>provider.sponsor_kind==='ADVERTISER'
      ?<article key={`advertiser-${provider.sponsor_position}-${provider.name}`} className="expo-sponsored-card advertiser"><span className="expo-sponsor-mark" aria-hidden="true"><Building2/></span><span><small><Text message="Advertiser"/></small><strong>{provider.name}</strong><p>{provider.description}</p><span className="expo-sponsored-actions">{provider.website_url?<a href={provider.website_url} target="_blank" rel="noreferrer"><ExternalLink aria-hidden="true"/><Text message="Website"/></a>:null}{provider.phone?<a href={`tel:${provider.phone}`}><Phone aria-hidden="true"/><Text message="Call"/></a>:null}</span></span></article>
      :<article key={`transporter-${provider.sponsor_position}-${provider.handle}`} className="expo-sponsored-card"><ProviderIdentity provider={provider}/><span><small><Text message="Transporter"/></small><strong>{provider.name}</strong><span className="expo-sponsored-actions"><Link href={`/@${encodeURIComponent(provider.handle)}`}><Building2 aria-hidden="true"/><Text message="Profile"/></Link><Link href={`/?q=${encodeURIComponent(provider.name)}`}><Truck aria-hidden="true"/><Text message="Trucks"/></Link></span></span></article>)}</div>
    <p><ShieldCheck aria-hidden="true"/><Text message="Confirm sponsor claims and terms directly."/></p>
  </aside>;
}
...
export function TransportExpoVenue({providers,sponsoredProviders=[],featureDate,initialSchedule,week=[],expoGroup,tiktokUrl}:{providers:any[];sponsoredProviders?:any[];featureDate:string;initialSchedule:any;week?:any[];expoGroup:any;tiktokUrl?:string|null}){
  ...
  return <div className={`transport-expo-experience featured-truck-experience${sponsoredProviders.length?'':' without-sponsors'}`}>
    <div className="featured-programme-command">
      <span className="featured-programme-current"><CalendarDays aria-hidden="true"/><span><small>{featuredDateLabel(featureDate)}</small><strong>{expoGroup.label} · {providers.length}<Text message=" trucks"/></strong></span></span>
      <details className="regional-expo-week-panel" name="featured-programme-panel"><Localized as="summary" copy={["aria-label"]} aria-label="View this week’s featured truck types"><CalendarDays aria-hidden="true"/><span><Text message="Week"/></span><ChevronDown aria-hidden="true"/></Localized><Localized as="div" copy={["aria-label"]} className="regional-expo-week" role="region" aria-label="Weekly featured truck types">{week.map((group:any)=><span className={group.key===expoGroup.key?'current':''} key={group.key}><small>{group.day.slice(0,3)} · {group.dateLabel}</small><strong>{group.shortLabel||group.label}</strong>{group.key===expoGroup.key?<em><Text message="Today"/></em>:null}</span>)}</Localized></details>
      <details className="expo-schedule-panel" name="featured-programme-panel"><summary className={`expo-live-strip ${schedule.phase}`} aria-label={`${stateLabel}. 07:30–09:00 EAT. View schedule`}><Clock3 aria-hidden="true"/><span>{stateLabel}</span><ChevronDown aria-hidden="true"/></summary><div className="expo-programme-detail"><strong>{currentIndex>=0?`Live · ${providers[currentIndex]?.cargo_configuration} with ${providers[currentIndex]?.driver_first_name}`:`Morning programme · ${stateLabel}`}</strong><small><Text message="07:30–09:00 EAT"/></small><Localized as="div" copy={["aria-label"]} className="expo-programme-strip" role="region" tabIndex={0} aria-label="Featured truck programme">{schedule.entries.map((entry:any,index:number)=><span key={`${entry.type}-${entry.starts_at}-${index}`} className={`${entry.type.toLowerCase().replace('_','-')}${active?.starts_at===entry.starts_at&&active?.type===entry.type?' active':''}`}><small>{entry.time_label}</small><strong>{entry.type==='PROVIDER'?providers.find(provider=>provider.truck_key===entry.provider_key)?.cargo_configuration:entry.label}</strong></span>)}</Localized></div></details>
      {tiktokUrl?<a className="featured-programme-action live" href={tiktokUrl} target="_blank" rel="noreferrer"><Play aria-hidden="true"/><span><Text message="Live"/></span><ExternalLink aria-hidden="true"/></a>:null}
      <Localized as="link" copy={["aria-label"]} className="featured-programme-action" href="/" aria-label="Find open transport capacity"><Truck aria-hidden="true"/><span><Text message="Capacity"/></span></Localized>
    </div>
    <div className="transport-expo-venue"><div className="featured-board-stage" aria-label={`${expoGroup.label} featured truck board`}><div className="featured-board-surface"><div className="featured-board-grid">{providers.map((truck,index)=>{const walkthrough=walkthroughs.find((item:any)=>item.provider_key===truck.truck_key);const isCurrent=Boolean(walkthrough?.current);const truckName=truck.vehicle_label||[truck.vehicle_make,truck.vehicle_model].filter(Boolean).join(' ')||truck.cargo_configuration;return <button key={truck.truck_key} type="button" className={`featured-provider-tile featured-truck-tile${isCurrent?' walkthrough-current':''}${selectedIndex===index?' selected':''}`} onClick={event=>{selectedTriggerRef.current=event.currentTarget;setSelectedIndex(index);}} aria-label={`Driver ${truck.driver_first_name}, ${truck.driver_kind_label}, ${truck.cargo_configuration}, ${truck.name}, ${walkthrough?.label}`}><span className="featured-card-number">{String(index+1).padStart(2,'0')}</span><span className="featured-card-feature">{isCurrent?<Text message="Live"/>:<Text message="View"/>}</span><span className="featured-card-photo"><DriverPortrait truck={truck}/><span className="featured-card-truck"><Localized as="img" copy={["alt"]} src={vehicleConfigurationImage(truck.cargo_configuration)} alt=""/></span></span><span className="featured-card-details"><small>{truck.driver_kind_label}</small><strong>{truck.driver_first_name}</strong><small><Truck aria-hidden="true"/>{truck.cargo_configuration} · {truckName}</small><small><Building2 aria-hidden="true"/>{truck.name}</small><em>{walkthrough?.label}</em></span></button>;})}</div></div></div></div>
