...
function CapacityFeedState({initial,query,searchPath='/',apiPath='/api/public/capacity',onReset}:FeedProps&{onReset:()=>void}){
  ...
  const searchControls=<div className="capacity-search-controls">
    <div className="capacity-search-combobox" onBlur={event=>{
      ...
      {searchSuggestionsOpen?<div id="capacity-search-suggestions" className="capacity-search-suggestions" role="listbox" aria-label="Search suggestions">{searchSuggestionsLoading?<LoadingIndicator className="capacity-suggestion-loading" label="Finding trucks…"/>:searchSuggestions.length?searchSuggestions.map((suggestion:SearchSuggestion)=><Link key={suggestion.key} href={suggestion.href} role="option" onClick={()=>setSearchSuggestionsOpen(false)}><span className={`capacity-suggestion-icon ${suggestion.kind.toLowerCase()}`}>{suggestion.kind==='PROVIDER'?<Building2 aria-hidden="true"/>:<Truck aria-hidden="true"/>}</span><span><small>{suggestion.kind==='PROVIDER'?'Transporter':'Truck'}</small><strong>{suggestion.title}</strong><em>{suggestion.detail}</em></span></Link>):<span className="capacity-suggestion-state">No current trucks match that search.</span>}</div>:null}
    </div>
  </div>;
  ...
  return <div className="market-workbench market-map-view capacity-drawer-workbench">
    ...
    <dialog ref={filterDialog} className="capacity-filter-dialog" aria-labelledby="capacity-filter-title" onClose={()=>setFilterOpen(false)}>
      <div className="capacity-filter-dialog-head"><div><small>Find the right truck</small><h2 id="capacity-filter-title">More filters</h2></div><button type="button" onClick={closeFilters} aria-label="Close filters"><X aria-hidden="true"/></button></div>
      <div className="capacity-detail-fields">

        <fieldset><legend><Route aria-hidden="true"/>Route matching</legend>
    ...
    <aside ref={drawer} id="capacity-filter-drawer" className={`market-command-column capacity-filter-drawer${drawerOpen?' is-open':''}`} aria-label="Capacity filters" aria-hidden={!drawerOpen} inert={!drawerOpen} tabIndex={-1} onKeyDown={event=>{if(event.key==='Escape'&&!event.defaultPrevented&&!filterOpen){event.preventDefault();closeDrawer();}}}>
      <header className="capacity-drawer-heading" onPointerDown={startSwipe} onPointerUp={event=>finishSwipe(event,true)} onPointerCancel={()=>{swipeStart.current=null;}}>
        <div><small>Find a truck</small><h2>Filters</h2></div><button type="button" aria-label="Close filter drawer" onClick={closeDrawer}><PanelLeftClose aria-hidden="true"/></button>
      </header>
      <div className="capacity-drawer-scroll">
        {searchControls}
        <div className="capacity-primary-selects">
          <div className="form-group"><label htmlFor="capacity-main-status">Availability</label><select id="capacity-main-status" name="status" value={statusFilter} onChange={event=>chooseStatus(event.target.value)}><option value="">Empty or Partial</option><option value="EMPTY">Empty</option><option value="PARTIAL">Partial</option></select></div>
          <div className="form-group"><label htmlFor="capacity-main-truck">Truck configuration</label><select id="capacity-main-truck" name="vehicleCategory" value={vehicleFilter} onChange={event=>setVehicleFilter(event.target.value)}><option value="">Any configuration</option>{VEHICLE_CONFIGURATIONS.map(configuration=><option key={configuration.name} value={configuration.name}>{configuration.name}</option>)}</select></div>
        </div>
        <fieldset className="capacity-truck-location-filter">
          <legend><MapPinned aria-hidden="true"/>Truck location</legend>
          <p>Near the truck’s reported location, allowing for its location accuracy.</p>
          <div className="capacity-geography-filter">
            <div className="form-group"><label htmlFor="capacity-truck-city">In or near a city</label><EthiopiaPlaceInput id="capacity-truck-city" name="truckCity" placeRefName="truckCityPlaceRef" defaultPlaceRef={query.truckCityPlaceRef||''} defaultValue={query.truckCity||''} onChange={event=>{setTruckCityName(event.target.value);setNearFilterEnabled(false);}} placeholder="Choose a city or town"/></div>
            <div className="form-group"><label htmlFor="capacity-truck-location-radius">Distance from city</label><select id="capacity-truck-location-radius" name="truckLocationRadiusKm" disabled={!truckCityName.trim()&&!query.truckCityPlaceRef} defaultValue={query.truckLocationRadiusKm||'50'}>{[5,10,25,50,100,200,300].map(value=><option value={value} key={value}>Within {value} km</option>)}</select></div>
          </div>
          <label className="checkbox-label" htmlFor="capacity-near-enabled"><input id="capacity-near-enabled" type="checkbox" checked={nearFilterEnabled} disabled={!near||Boolean(truckCityName.trim())} onChange={event=>setNearFilterEnabled(event.target.checked)}/><span>Show trucks near my area</span></label>
          ...
          <p>{truckCityName.trim()?'Clear the city to use your area instead.':near?'Your precise location stays on this device.':'You can choose a city without sharing your location.'}</p>
        </fieldset>

<fieldset className="capacity-route-filter"><legend><Route aria-hidden="true"/>Shipment route</legend><div className="capacity-main-route">            <div className="form-group"><label htmlFor="capacity-route-origin"><MapPinned aria-hidden="true"/>Origin <small>(optional)</small></label><EthiopiaPlaceInput id="capacity-route-origin" name="origin" placeRefName="originPlaceRef" defaultPlaceRef={query.originPlaceRef||''} defaultValue={query.origin||''} placeholder="City or town"/></div>
