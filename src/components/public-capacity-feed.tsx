...
import { Building2, CalendarClock, ChevronDown, LocateFixed, MapPinned, Phone, RefreshCw, Search, SlidersHorizontal, Truck, UserRound, X } from 'lucide-react';
...
export function PublicCapacityFeed({initial,query,searchPath='/',apiPath='/api/public/capacity'}:{initial:FeedResult;query:Record<string,string>;searchPath?:string;apiPath?:string}){
  ...
  return <>
    ...
    {!items.length&&!loading?<div className="empty-state">No truck signals match these filters. Adjust your search or check again later.</div>:null}
    ...
    <div className="public-feed-end">{hasMore?<button type="button" className="button secondary" onClick={()=>void loadMore()} disabled={loading}><ChevronDown aria-hidden="true"/>{loading?'Loading more…':'Load more trucks'}</button>:items.length?<span>All matching truck signals are shown.</span>:null}</div></div>
  </>;
}
