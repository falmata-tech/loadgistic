...
export function VerificationForm({subjects}:{subjects:Subject[]}) {
  ...
  return <form action="/api/verifications" method="post" encType="multipart/form-data" className="form-card stack">
    ...
    <div className="form-grid">
      ...
      {verificationType==='VEHICLE_AUTHORIZATION'?<><div className="form-group"><label htmlFor="related-vehicle">Authorized truck</label><select id="related-vehicle" name="relatedVehicleId" required>{selected?.vehicles?.map(vehicle=><option value={vehicle.id} key={vehicle.id}>{vehicle.label}</option>)}</select></div><div className="form-group"><label htmlFor="authorization-expiry">Authorization expires</label><input id="authorization-expiry" name="expiresOn" type="date" min={new Date(Date.now()+86_400_000).toISOString().slice(0,10)} required/></div></>:null}
