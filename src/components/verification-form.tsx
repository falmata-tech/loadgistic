...
export function VerificationForm({subjects}:{subjects:Subject[]}) {
  ...
  return <form action="/api/verifications" method="post" encType="multipart/form-data" className="form-card stack">
    ...
    <div className="form-grid">
      ...
      <div className="form-group"><label htmlFor="verification-type"><ShieldCheck aria-hidden="true"/>Verification type</label><select id="verification-type" name="verificationType" key={key} required>{selected?.allowed_types.map(type=><option value={type} key={type}>{labels[type]||type}</option>)}</select></div>
