  ...
  return <form action="/api/shipments" method="post" className="load-composer stack" data-testid="new-shipment-form">
    ...
    <section className="load-composer-section">
      ...
      <div className="form-grid">
        ...
        <div className="route-inputs full"><div className="form-group"><label htmlFor="shipment-origin"><MapPin aria-hidden="true"/>From city</label><input id="shipment-origin" name="origin" required placeholder="Addis Ababa"/></div><div className="route-arrow" aria-hidden="true"><ArrowRightLeft/></div><div className="form-group"><label htmlFor="shipment-destination"><MapPin aria-hidden="true"/>To city</label><input id="shipment-destination" name="destination" required placeholder="Hawassa"/></div></div>
    ...
    <section className="load-composer-section">
      ...
      <div className="form-grid">
        <fieldset className="form-group full"><legend><Share2 aria-hidden="true"/>Load visibility</legend><div className="rich-choice-grid three"><label className="rich-choice"><input type="radio" name="distributionMode" value="OPEN_MARKET" checked={distributionMode==='OPEN_MARKET'} onChange={event=>setDistributionMode(event.target.value)}/><Search aria-hidden="true"/><span><strong>Load Board</strong><small>All approved transporters</small></span><Check className="choice-check" aria-hidden="true"/></label><label className="rich-choice"><input type="radio" name="distributionMode" value="SAVED_PARTNERS" checked={distributionMode==='SAVED_PARTNERS'} onChange={event=>setDistributionMode(event.target.value)}/><ShieldCheck aria-hidden="true"/><span><strong>Partners</strong><small>Saved relationships only</small></span><Check className="choice-check" aria-hidden="true"/></label><label className="rich-choice"><input type="radio" name="distributionMode" value="DIRECT_TO_PROVIDER" checked={distributionMode==='DIRECT_TO_PROVIDER'} onChange={event=>setDistributionMode(event.target.value)}/><Send aria-hidden="true"/><span><strong>Direct</strong><small>One transporter</small></span><Check className="choice-check" aria-hidden="true"/></label></div></fieldset>
