...
type TruckPreview={status:string;available_percent?:number|null;available_again_date?:string|null;movement_scope?:string|null;local_place_label?:string|null;local_radius_km?:number|null;location_area?:string|null;location_precision_km?:number|null;current_route_origin?:string|null;current_route_destination?:string|null;current_route_date?:string|null;origin?:string|null;destination?:string|null;travel_date?:string|null;planned_space_status?:string|null;vehicle_make?:string|null;vehicle_model?:string|null;cargo_configuration?:string|null;accepts_full_load:boolean;accepts_partial_load:boolean;accepts_multi_pick:boolean;accepts_multi_drop:boolean;open_to_contract_lanes:boolean;proof_available:boolean;freshness?:string|null;updated_label?:string|null};
...
function acceptedLoads(truck:TruckPreview){
  if(truck.accepts_full_load&&truck.accepts_partial_load)return 'FTL + PTL';
  if(truck.accepts_partial_load)return 'PTL';
  return 'FTL';
}
...
function truckRoute(truck:TruckPreview){
  ...
  if(truck.movement_scope==='BOTH'&&truck.local_place_label)return `Local + between cities · ${truck.local_place_label}`;
    ...
    <div className="public-board-shell">
      ...
      <div className="public-board-list" role="tabpanel">
        {active==='SHIPMENTS'?(shipments as ShipmentPreview[]).map((shipment,index)=><article className="locked-board-card rich-preview-card" key={index}>
          ...
          <div className="preview-tags"><span>{shipment.movement_scope==='LOCAL'?'Local':'Between cities'}</span><span>{shipment.distribution_mode==='OPEN_MARKET'?'Public':'Limited'}</span><span>Posted {shipment.posted_label||'recently'}</span></div>
          ...
          <div className="preview-tags"><span>{truck.movement_scope==='BOTH'?'Local + between cities':truck.movement_scope==='LOCAL'?'Local':'Between cities'}</span><span>{truck.proof_available?'Photo recorded':'No photo'}</span><span>{truck.open_to_contract_lanes?'Contract routes':'Single trip'}</span>{truck.status==='BUSY'&&truck.available_again_date?<span>Ready {truck.available_again_date}</span>:null}</div>
          <div className="preview-card-actions"><Link href="/login" className="button secondary"><Eye aria-hidden="true"/>Details</Link><Link href="/login" className="button"><UserRound aria-hidden="true"/>Contact<ArrowRight aria-hidden="true"/></Link></div>
        </article>)}
