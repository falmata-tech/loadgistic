...
import { ArrowRight, Banknote, Boxes, CalendarClock, CircleDotDashed, Clock3, Eye, Gauge, Layers3, LockKeyhole, MapPin, MapPinned, Route, ShieldCheck, Truck, UserRound, Waypoints } from 'lucide-react';
...
function truckRoute(truck:TruckPreview){
  ...
  return truck.status==='EMPTY'?'Willing to go anywhere':'Preferred route available after login';
}

export function PublicBoardPreview({shipments,trucks,shared}:{shipments:ShipmentPreview[];trucks:TruckPreview[];shared:SharedPreview}){
  ...
  return <section className="public-board-section" aria-labelledby="marketplace-preview-title"><div className="container">
    <div className="public-board-heading"><div><span className="eyebrow"><Eye aria-hidden="true"/>Live marketplace</span><h2 id="marketplace-preview-title">Shipments meet available trucks.</h2></div><p>Real routes and matching signals. Member identity and contact stay protected.</p></div>
    <div className="public-board-shell">
      ...
      <div className="public-board-list" role="tabpanel">
        {active==='SHIPMENTS'?<>
          ...
          <div className="locked-card-facts"><div><Truck aria-hidden="true"/><span><small>Cargo configuration</small><strong>{truck.cargo_configuration||'Truck configuration'}</strong></span></div><div><MapPin aria-hidden="true"/><span><small>Current area</small><strong>{truck.location_area||truck.local_place_label||'General area shared'}</strong></span></div><div><Boxes aria-hidden="true"/><span><small>{truck.status==='BUSY'?'Available near':'Accepting'}</small><strong>{truck.status==='BUSY'?truck.available_again_place_label||'Expected area pending':`${acceptedLoads(truck)} · ${stopPolicy(truck)}`}</strong></span></div><div><Clock3 aria-hidden="true"/><span><small>Freshness</small><strong>{truck.updated_label||truck.freshness||'Recently updated'}</strong></span></div></div>
