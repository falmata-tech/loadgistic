...
export function PublicAssistedChat(){
  ...
  return <>
    <button className="public-chat-launcher" type="button" disabled={!hydrated} onClick={()=>setOpen(true)} aria-haspopup="dialog" aria-label={conversation?.unread_team_count?`Ask Loadgistic, ${conversation.unread_team_count} unread message${conversation.unread_team_count===1?'':'s'}`:'Ask Loadgistic'}>
      <Headphones aria-hidden="true"/><span><Text message="Ask Loadgistic"/></span>{conversation?.unread_team_count?<b>{conversation.unread_team_count}</b>:null}
    </button>
    <dialog ref={dialog} className="public-chat-dialog" aria-labelledby="public-chat-title" onClose={()=>setOpen(false)} onCancel={event=>{event.preventDefault();setOpen(false);}}>
      <header><div><span>{presence.available?<Text message="Team available"/>:<Text message="Leave a message"/>}</span><h2 id="public-chat-title"><Text message="Ask Loadgistic"/></h2></div><Localized as="button" copy={["aria-label"]} type="button" onClick={()=>setOpen(false)} aria-label="Minimize chat"><X aria-hidden="true"/></Localized></header>
      {loading?<SurfaceSkeleton kind="chat" className="public-chat-loading" label="Opening your conversation"/>:conversation&&!newSession?<div className="public-chat-conversation">
        <div className={`public-chat-presence ${conversation.assigned_agent_name?'online':'waiting'}`}><span aria-hidden="true"/><span>{conversation.assigned_agent_name?`${conversation.assigned_agent_name} is helping`:<Text message="Waiting for a team member"/>}</span>{conversation.status!=='CLOSED'?<button type="button" onClick={endChat}>{confirmEnd?<Text message="Confirm end"/>:<Text message="End chat"/>}</button>:null}</div>
        {conversation.has_older||conversation.history_before?<Localized as="nav" copy={["aria-label"]} className="support-history-nav" aria-label="Message history">
          {conversation.has_older?<button className="button secondary small" type="button" disabled={busy} onClick={()=>void showHistory(conversation.next_before||'')}><Text message="Older messages"/></button>:<span><Text message="Beginning of conversation"/></span>}
          {conversation.history_before?<button className="button secondary small" type="button" disabled={busy} onClick={()=>void showHistory('')}><Text message="Latest messages"/></button>:null}
          {conversation.history_before?<small><Text message="Earlier messages · updates paused"/></small>:null}
        </Localized>:null}
        <div className="public-chat-messages" aria-live="polite">{conversation.messages.map((message:Message)=><article className={message.sender_kind==='GUEST'?'mine':''} key={message.id}><strong>{message.sender_kind==='GUEST'?<Text message="You"/>:<Text message="Loadgistic team"/>}</strong><p>{message.body}</p>{message.attachment_id?<a href={`/api/guest-support/${conversation.id}/attachments/${message.attachment_id}`}><Paperclip aria-hidden="true"/>{message.attachment_name}</a>:null}<time dateTime={message.created_at}>{new Date(message.created_at).toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</time></article>)}</div>
        {conversation.status!=='CLOSED'?<form className="public-chat-composer" onSubmit={reply}><label className="sr-only" htmlFor="public-chat-reply"><Text message="Reply"/></label><Localized as="textarea" copy={["placeholder"]} id="public-chat-reply" name="body" rows={2} maxLength={2000} required placeholder="Write a reply…"/><Localized as="label" copy={["title"]} className="public-chat-file" title="Attach a requested file"><Paperclip aria-hidden="true"/><span>{attachmentName||'Attach file'}</span><input type="file" name="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={event=>setAttachmentName(event.target.files?.[0]?.name||'')}/></Localized><Localized as="button" copy={["aria-label"]} type="submit" aria-label="Send reply"><Send aria-hidden="true"/><span><Text message="Send"/></span></Localized></form>:<div className="public-chat-closed"><ShieldCheck aria-hidden="true"/><span><Text message="This chat has ended. The transcript is retained for follow-up."/></span><button className="button small" type="button" onClick={()=>setNewSession(true)}><Text message="Start a new chat"/></button></div>}
      </div>:<form className="public-chat-start" onSubmit={start}><p><Text message="Tell our team where your cargo needs to move. We use your email and phone only to continue helping if the chat disconnects."/></p><label><Text message="Email"/><input disabled={busy} name="email" type="email" autoComplete="email" required/></label><label><Text message="Callback phone"/><input disabled={busy} name="phone" type="tel" autoComplete="tel" required/></label><label><Text message="What do you need?"/><Localized as="textarea" copy={["placeholder"]} disabled={busy} name="body" rows={4} maxLength={2000} required placeholder="For example: I need a cargo van from Adama to Bishoftu tomorrow."/></label><button className="button" type="submit" disabled={busy}><MessageCircle aria-hidden="true"/><Text message="Start chat"/></button><Link className="public-chat-recovery" href="/help"><Text message="Return with a recovery code"/></Link></form>}
      {error?<p className="public-chat-error" role="alert">{error}</p>:null}
      {refreshError?<p className="public-chat-error" role="status">{refreshError} <button type="button" disabled={busy} onClick={()=>void refresh(open)}><Text message="Retry updates"/></button></p>:null}
    </dialog>
  </>;
}
