  ...
  return <section className="profile-route-editor">
    ...
    <div className="profile-route-rows">
      {routes.map((route,index)=><div className="profile-route-row" key={route.id||index}>
        <div className="form-group"><label htmlFor={`route-origin-${index}`}>City 1</label><EthiopiaPlaceInput id={`route-origin-${index}`} name="routeOrigin" placeRefName="routeOriginPlaceRef" defaultPlaceRef={route.origin_place_ref||''} defaultValue={route.origin} placeholder="Addis Ababa, Ethiopia"/></div>
        <span className="route-pair-arrow" aria-hidden="true">↔</span>
        <div className="form-group"><label htmlFor={`route-destination-${index}`}>City 2</label><EthiopiaPlaceInput id={`route-destination-${index}`} name="routeDestination" placeRefName="routeDestinationPlaceRef" defaultPlaceRef={route.destination_place_ref||''} defaultValue={route.destination} placeholder="Hawassa, Ethiopia"/></div>
