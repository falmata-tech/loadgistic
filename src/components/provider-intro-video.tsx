...
export function ProviderIntroVideo({videoId,providerName}:{videoId:string;providerName:string}){
  ...
  return <div className="provider-video">{playing?<iframe src={`https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1`} title={`${providerName} introduction`} allow="accelerometer; autoplay; encrypted-media; picture-in-picture" allowFullScreen/>:<button type="button" onClick={()=>setPlaying(true)} style={{backgroundImage:`linear-gradient(90deg,rgba(3,20,33,.82),rgba(3,20,33,.2)),url(https://i.ytimg.com/vi/${videoId}/hqdefault.jpg)`}}><Play aria-hidden="true"/><span><strong><Text message="Meet "/>{providerName}</strong><small><Text message="Play provider introduction"/></small></span></button>}</div>;
}
