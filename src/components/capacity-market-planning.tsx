...
export function CapacityMarketPlanning({corridors,returnTo,onBusyChange,onSaved,onRemoved,onCancel}:{corridors:RegularSignal[];returnTo:string;onBusyChange:(busy:boolean)=>void;onSaved:()=>void;onRemoved:()=>void;onCancel:()=>void}){
  ...
  return <section className="capacity-signal-form" data-testid="capacity-planning-editor">
    ...
    <p className="meta">The area or two-way route you serve regularly. Customers still confirm current availability with you.</p>
    ...
    <form id="regular-service-form" action="/api/capacity/corridors" method="post" onSubmit={submit} className="planning-editor-form recurring-signal-form"><fieldset className="capacity-signal-fields" disabled={busy}><input type="hidden" name="returnTo" value={returnTo}/><input type="hidden" name="action" value={published?'UPDATE':'ADD'}/><input type="hidden" name="id" value={published?.id||''}/><input type="hidden" name="geometry" value={geometry}/>
      <div className="segmented-control geometry-choice"><button type="button" aria-pressed={geometry==='RADIUS'} onClick={()=>setGeometry('RADIUS')}><CircleDotDashed aria-hidden="true"/><strong>Service area</strong><small>Nearby towns</small></button><button type="button" aria-pressed={geometry==='ROUTE'} onClick={()=>setGeometry('ROUTE')}><Route aria-hidden="true"/><strong>Capacity route</strong><small>Cities in travel order</small></button></div>
      {geometry==='ROUTE'?<PlaceRows keys={routeKeys} setKeys={setRouteKeys} prefix="regular-route" label="Route city" min={MIN_ROUTE_POINTS} name="routePlace" refName="routePlaceRef" points={corridors[0]?.route_points}/>:<><div className="form-group"><label htmlFor="regular-area-center"><MapPin aria-hidden="true"/>Area center</label><EthiopiaPlaceInput id="regular-area-center" name="areaCenter" placeRefName="areaCenterPlaceRef" defaultValue={corridors[0]?.area_center_label} defaultPlaceRef={corridors[0]?.area_center_place_ref} required/></div><PlaceRows keys={areaKeys} setKeys={setAreaKeys} prefix="regular-area" label="Surrounding city" min={MIN_AREA_POINTS} name="areaBoundaryPlace" refName="areaBoundaryPlaceRef" points={corridors[0]?.area_boundary}/></>}
      </fieldset>
    </form>
    {published?<form action="/api/capacity/corridors" method="post" onSubmit={submit}><input type="hidden" name="returnTo" value={returnTo}/><input type="hidden" name="action" value="REMOVE"/><input type="hidden" name="id" value={published.id}/><button className="button secondary small" aria-label="Remove regular service" disabled={busy}><Trash2 aria-hidden="true"/>Remove regular service</button></form>:null}
    </div>
    <footer className="capacity-signal-dialog-footer"><button type="button" className="button secondary" disabled={busy} onClick={onCancel}>Cancel</button><button className="button" form="regular-service-form" disabled={busy}><Save aria-hidden="true"/>{busy?'Saving…':'Save'}</button></footer>
  </section>;
}
