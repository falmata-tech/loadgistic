...
export function AccessControlsForm({controls}:{controls:Controls}){
  ...
  return <form action="/api/admin/settings" method="post" className="card platform-control-card">
    ...
    <header><Settings2 aria-hidden="true"/><div><h2><Text message="Workspace access"/></h2><p><Text message="Choose when trials and payments begin."/></p></div></header>
    <fieldset className="platform-mode-options"><legend className="sr-only"><Text message="Access mode"/></legend>
      <label className={mode==='FREE'?'selected':''}><input type="radio" name="mode" value="FREE" checked={mode==='FREE'} onChange={()=>setMode('FREE')}/><span><strong><Text message="Free access"/></strong><small><Text message="No trial countdown or payment required."/></small></span></label>
      <label className={mode==='TRIAL_PAYMENT'?'selected':''}><input type="radio" name="mode" value="TRIAL_PAYMENT" checked={mode==='TRIAL_PAYMENT'} onChange={()=>setMode('TRIAL_PAYMENT')}/><span><strong><Text message="Trial, then payment"/></strong><small><Text message="Seven days free, then an active paid plan."/></small></span></label>
    </fieldset>
    {activating?<label className="platform-activation-confirm"><input type="checkbox" name="confirm" value="ENABLE" required/><span><Text message="Start a fresh seven-day trial for existing unpaid providers. Payment will be required afterwards."/></span></label>:null}
    <p className="meta"><Text message="Existing payments and account history are kept. Changing this never charges anyone automatically."/></p>
    <button className="button"><CheckCircle2 aria-hidden="true"/><Text message="Save access mode"/></button>
  </form>;
}
