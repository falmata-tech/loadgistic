...
export function FeaturedProviderSection({feature}:{feature:any}){
  ...
  return <section id="featured-providers" className="featured-provider-section" aria-labelledby="featured-provider-title"><div className="container regional-expo-shell">
    <header className="regional-expo-intro"><div><span className="section-kicker"><Text message="Featured"/></span><h2 id="featured-provider-title">{headline}</h2><p>{feature.introduction}</p></div></header>
    {providerCount?<TransportExpoVenue providers={feature.providers} sponsoredProviders={feature.sponsored_providers||[]} featureDate={feature.feature_date} initialSchedule={feature.schedule} week={feature.week} expoGroup={feature.expo_group} tiktokUrl={feature.tiktok_url}/>:<div className="regional-expo-empty"><div><h3><Text message="No trucks are scheduled today."/></h3><p><Text message="You can still explore trucks with published capacity."/></p></div><Link className="button secondary" href="/"><Text message="Find capacity"/></Link></div>}
    <p className="featured-provider-note"><ShieldCheck aria-hidden="true"/><Text message="Confirm current documents, capacity, cargo fit, price, and timing directly."/></p>
  </div></section>;
}
