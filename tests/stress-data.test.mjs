...
test('standard stress dataset covers every table, workflow state, and provider scope',()=>{
  ...
  assert.equal(operations.users.length,10);
  assert.equal(operations.vehicles.length,10);
  assert.equal(operations.loads.length,10);
  assert.ok(operations.pagination.users.pageCount>20);
  assert.ok(operations.pagination.loads.pageCount>50);
  const operationsPageTwo=repo.getAdminOperations(admin,'',{userPage:2,truckPage:2,loadPage:2,capacityPage:2,workspacePage:2});
  assert.equal(operationsPageTwo.pagination.users.page,2);
  assert.equal(operations.users.some(first=>operationsPageTwo.users.some(second=>second.id===first.id)),false);
