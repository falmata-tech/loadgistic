...
async function assertReadableFeedback(page:any){
  await expect.poll(()=>page.evaluate(()=>{
    ...
    return {
      clearIdentityAndActions:[...document.querySelectorAll('.map-truck-identity,.map-focus-exit,.map-capacity-sheet>.public-card-actions')].every(element=>!overlaps(feedback,element.getBoundingClientRect())),
      contained:sheet.top>=canvas.top&&sheet.bottom<=canvas.bottom&&sheet.left>=canvas.left&&sheet.right<=canvas.right,
      feedbackReadable:feedback.left>=canvas.left&&feedback.right<=canvas.right&&feedback.top>=canvas.top&&feedback.bottom<=canvas.bottom,
  ...
  })).toEqual({clearIdentityAndActions:true,contained:true,feedbackReadable:true,zoomReachable:true,noOverflow:true});
  await expect(page.locator('.public-feed-overlay')).toHaveCount(1);
}
