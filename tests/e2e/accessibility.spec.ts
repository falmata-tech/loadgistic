...
test('public filters support keyboard entry, Escape, and trigger focus restoration', async ({ page }: { page: Page }) => {
  ...
  await expectMinimumTarget(dialog.getByRole('button',{name:'Show matching trucks'}),'filter submit');
  const dialogBox=await dialog.boundingBox();
  const viewport=page.viewportSize();
  expect(dialogBox).not.toBeNull();
  expect(viewport).not.toBeNull();
  expect(dialogBox!.x,'filter dialog starts outside the viewport').toBeGreaterThanOrEqual(0);
  expect(dialogBox!.x+dialogBox!.width,'filter dialog ends outside the viewport').toBeLessThanOrEqual(viewport!.width+1);
  expect(dialogBox!.y,'filter dialog starts outside the viewport').toBeGreaterThanOrEqual(0);
  expect(dialogBox!.y+dialogBox!.height,'filter dialog ends outside the viewport').toBeLessThanOrEqual(viewport!.height+1);
