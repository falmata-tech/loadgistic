...
test('shipper can open rich shipment posting workflow', async ({ page }: { page: any }) => {
  ...
  for(const heading of ['Shipment','Route and dates','Truck needed','Who can see it?','Price'])await expect(page.getByRole('heading',{name:heading,exact:true})).toBeVisible();
  ...
  await expect(page.getByLabel('Tracking after assignment')).toHaveCount(0);
...
test('Business manages posting and active Tracking from one My Shipments workspace',async({page}:{page:any})=>{
  ...
  await expect(page.getByRole('navigation',{name:'Workspace navigation'}).getByRole('link',{name:'Tracking',exact:true})).toHaveCount(0);
  await expect(page.getByRole('link',{name:'Post shipment'})).toBeVisible();
  await expect(page.getByText('Construction materials to Mekelle')).toBeVisible();
...
test('native support carries one private conversation from customer to agent and admin',async({page}:{page:any})=>{
  ...
  await login(page,'transporter@loadgistic.local');
  await page.goto('/app/support');
  await expect(page.getByRole('link',{name:'Continue chat'}).first()).toBeVisible();
  await expect(page.getByRole('link',{name:'New chat'})).toHaveCount(0);
  await page.getByRole('link',{name:'Continue chat'}).first().click();
  ...
  await expect(page.getByRole('navigation',{name:'Conversation queue filter'})).toBeVisible();
...
test('fleet transporter lands on a management dashboard and updates capacity in My Fleet', async ({ page }: { page: any }) => {
  ...
  await expect(page.locator('.fleet-driver-manager summary').getByText('Yonas Alemu',{exact:true})).toBeVisible();
  await expect(page.locator('.fleet-truck-row').first().getByText('Isuzu · FSR',{exact:true})).toBeVisible();
  await expect(page.getByTestId('capacity-form')).toHaveCount(0);
  ...
  await expect(page.getByText(/Location comes from the assigned Driver/)).toBeVisible();
  ...
  for(const heading of ['Truck status','Work area','Shipment size','Stops accepted','Regular work','Share update'])await expect(page.getByRole('heading',{name:heading,exact:true})).toBeVisible();
  await page.getByRole('button',{name:'Partial',exact:true}).click();
  await expect(page.getByText('Space available',{exact:true})).toBeVisible();
  await expect(page.getByRole('heading',{name:'Live route',exact:true})).toBeVisible();
  await expect(page.getByRole('radio',{name:/Full Truckload/})).toHaveCount(0);
  await expect(page.getByRole('radio',{name:/Partial Truckload/})).toHaveCount(0);
  await expect(page.getByText(/Partial Truckload \(PTL\).*selected automatically/)).toBeVisible();
  await expect(page.getByText('Cargo photo', { exact:false })).toBeVisible();
...
test('fleet owner can reduce a company driver to duty-only Home and restore access', async ({ page }: { page: any }) => {
  ...
  await login(page,'transporter@loadgistic.local');
  await page.goto('/app/fleet');
  await page.locator('.fleet-driver-manager').filter({hasText:'Yonas Alemu'}).locator('summary').click();
  const driverForm=page.locator('form').filter({hasText:'Yonas Alemu'});
  await expect(driverForm.getByLabel('Truck')).toHaveValue('veh-trans-1');
  ...
  await driverForm.getByRole('checkbox',{name:'Agree + assign shipments'}).uncheck();
  await driverForm.getByRole('checkbox',{name:'Capacity'}).uncheck();
  await driverForm.getByRole('button',{name:'Save driver'}).click();
  await expect(page.getByText('Driver and truck updated.')).toBeVisible();
  ...
  for(const name of ['Shipment Board','Call Businesses','Agree + assign shipments','Capacity'])await restoreForm.getByRole('checkbox',{name}).check();
  await restoreForm.getByRole('button',{name:'Save driver'}).click();
  await expect(page.getByText('Driver and truck updated.')).toBeVisible();
});
...
test('self-managed driver keeps the rich capacity control panel as Home', async ({ page }: { page: any }) => {
  ...
  await expect(page.getByText(/Partial Truckload \(PTL\).*selected automatically/)).toBeVisible();
  await expect(page.getByRole('heading',{name:'Shipment size',exact:true})).toHaveCount(0);
  ...
  await expect(page.getByLabel('Available near')).toBeVisible();
  await expect(page.getByRole('heading',{name:'Shipment size',exact:true})).toHaveCount(0);
  await expect(page.getByRole('heading',{name:'Stops accepted',exact:true})).toHaveCount(0);
  await expect(page.getByRole('heading',{name:'Regular work',exact:true})).toBeVisible();
  await expect(page.getByRole('heading',{name:'Share update',exact:true})).toBeVisible();
});
...
test('member verification center and admin review queue are available', async ({ page }: { page: any }) => {
  ...
  if((page.viewportSize()?.width||1000)<980){
    await page.getByLabel('Management area').selectOption('TRUCKS');
...
test('provider interest stays marked on the Board and appears under My Shipments Interested',async({page}:{page:any})=>{
  ...
  await expect(page.getByText('Beverage shipment to Dire Dawa')).toBeVisible();
  await page.goto('/app/shipments');
  await page.getByRole('link',{name:'Interested',exact:true}).click();
  await expect(page.getByText('Beverage shipment to Dire Dawa')).toBeVisible();
  await page.getByRole('link',{name:'Tracking',exact:true}).click();
  await expect(page.getByText('Beverage shipment to Dire Dawa')).toHaveCount(0);
});
