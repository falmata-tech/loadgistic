...
for(const kind of ['independent','company'])test(`public ${kind} fleet pages reach every truck with scoped capacity`,async({page}:{page:any},info:any)=>{
  test.setTimeout(150000);const service=localService();const suffix=randomUUID().slice(0,8);const handle=`paging-${suffix}`;
  const count=kind==='independent'?105:13;let actor='';let provider='';let org='';let vehicles:any[]=[];
  ...
  try{
    ...
    if(kind==='independent')provider=checked(await service.from('provider_profiles').insert({user_id:actor,business_name:`Paging ${suffix}`,handle,city:'Addis Ababa'}).select('id').single()).id;
    else{
    ...
    if(provider){
      ...
      checked(await service.from('verification_requests').insert({subject_type:'VEHICLE',subject_id:vehicles[99].id,verification_type:'VEHICLE_OWNERSHIP',document_name:'Synthetic paging evidence',storage_path:'synthetic-private-paging-proof',original_name:'fixture.txt',mime_type:'text/plain',status:'APPROVED',submitted_by:actor}));
    }
    const pages=Math.ceil(count/12);await page.goto(`/@${handle}`);
    await expect(page.locator('.provider-hero-facts')).toContainText(`${count} active trucks`);
    ...
    for(let current=1;current<=pages;current++){
      await expect(nav).toContainText(`Page ${current} of ${pages} · ${count} results`);
      ...
      if(current<pages){await nav.getByRole('link',{name:'Next page',exact:true}).click();await expect(page).toHaveURL(new RegExp(`/@${handle}\\?truckPage=${current+1}$`));}
    }
