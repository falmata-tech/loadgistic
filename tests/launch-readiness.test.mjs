...
test('local readiness is runnable and production readiness names durable blockers',()=>{
  ...
  for(const blocker of ['strong-session-secret','durable-private-storage','managed-postgres-data-backend','managed-auth-callback-url','supabase-public-config','supabase-service-config','managed-email-delivery','supabase-repository-adapter','managed-identity-adapter','shared-rate-limit-adapter','upload-malware-scanner']){
...
test('local managed runtime does not claim SQLite or silently accept missing service configuration',()=>{
  const configured=launchReadiness({NODE_ENV:'development',DATA_BACKEND:'supabase',PRIVATE_STORAGE_BACKEND:'supabase',NEXT_PUBLIC_SUPABASE_URL:'http://127.0.0.1:55321',NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY:'public',SUPABASE_SERVICE_ROLE_KEY:'service'});
  assert.equal(configured.ok,true);
  assert.ok(!configured.warnings.includes('local-sqlite-data'));
  const missingService=launchReadiness({NODE_ENV:'development',DATA_BACKEND:'supabase',PRIVATE_STORAGE_BACKEND:'local'});
