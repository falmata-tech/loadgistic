...
test('local readiness is runnable and production readiness names durable blockers',()=>{
  ...
  for(const blocker of ['strong-session-secret','durable-private-storage','managed-auth-callback-url','supabase-public-config','supabase-service-config','managed-email-delivery','shared-rate-limit-adapter','upload-malware-scanner']){
...
test('production requires managed transactional email and exposes only its provider name',()=>{
  const common={
    ...
    SESSION_SECRET:'a-secure-production-session-secret-that-is-long',NEXT_PUBLIC_SUPABASE_URL:'https://example.supabase.co',
