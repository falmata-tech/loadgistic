...
behavior: A Business, Fleet Transporter, or Self-managed Driver may privately favorite an eligible member on the other side of the market, request a connection, and accept or decline an incoming request; only a mutual Connected relationship unlocks Partners-only loads and capacity.
contracts: [NetworkRelationship, NetworkFavorite, ConnectionRequest, ConnectionDecision, PartnerVisibilityPolicy, NetworkDirectoryAction]
...
### Scenario: same-side and company-driver mutations are denied

Given a Business targets another Business, a provider targets another provider, or a company driver attempts to change the fleet network\
