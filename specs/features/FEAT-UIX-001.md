...
behavior: Every non-admin workflow uses a consistent icon-first visual language, short action labels, task-shaped grouping, touch-sized controls, visible state, and reversible navigation while retaining essential safety, privacy, and freight terminology.
contracts: [IconAction, TaskHeader, VisualChoice, TouchTarget, FactBlock, ReversibleDetail, EssentialHelpText]
