...
The local visual audit (`npm run test:ui-audit`) covers 146 logged-out and role-scoped screens at desktop and mobile sizes, including the producer homepage, Fleet Transporter dashboard, company Driver capacity Home, My Fleet permission and capacity controls, searchable boards, route evidence maps and comparison, Post Load, party-only Tracking, verification workflows, assigned-load controls, and authenticated tracking views. Its report checks response status, horizontal overflow, unlabeled controls, empty commands, and browser errors; screenshots and `report.json` are written to the ignored `artifacts/ui-audit/` directory.
