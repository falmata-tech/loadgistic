...
The runnable application uses Supabase for managed identity, managed provider signup, health, place search, the Truck Market, transporter microsites, Daily Featured Transporters, Shared capacity, provider Capacity, provider-owned Tracking, and shared abuse counters. Repository paths not yet migrated still have a Node SQLite compatibility adapter while ADR-041 is being completed; that is not an accepted Production fallback. Ordered SQL migrations `001` through `046` replay successfully against the isolated local Supabase PostgreSQL stack; additive migration `047` applied to that stack and passed its live verifier, while the next empty-database release rehearsal must replay the complete `001`–`047` chain. A guarded deterministic importer creates the complete fake market in local Supabase Auth, PostgreSQL, and private Storage, and managed identity/storage/RLS/workflow checks pass with `DATABASE_PATH=/dev/null`. Profile, fleet, verification, billing, Support, administration, and sponsorship management remain before Production readiness.
...
2. Apply `001` through `047` to an empty/staging project and run Supabase SQL lint plus schema/RLS review.
...
6. Configure private proof/support storage, malware scanning/quarantine, the verified Resend sender, authorized Supabase Realtime with polling fallback, Preview concurrency verification for shared rate limiting, and monitoring for scheduled email/cleanup jobs.
