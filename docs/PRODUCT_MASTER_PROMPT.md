...
Loadgistic is Ethiopia's transport-capacity sharing and shipment-tracking platform. Transporters share capacity signals with known brokers and enterprises shipping or receiving goods, or publish them to the open market. Once work is agreed, transporters provide professional private shipment tracking to brokers and their customers, or directly to enterprise contacts. Fleet transporters, owner-operators and self-managed Drivers control their capacity, business presentation and shipment updates.

## Audience and promise

The core relationship is between transport providers and the brokers, shippers and receivers they serve. Businesses of all sizes can discover capacity without creating an account; trusted email recipients access privately shared capacity and authorized shipment updates. These audience descriptions do not introduce broker or enterprise account roles. Optional document review gives people seeking transport evidence to assess the provider, Driver and truck. Loadgistic does not guarantee availability, price, performance, document validity or outcomes.

Public and workspace copy should reflect these relationships without repeating the full platform explanation. About carries the fuller narrative. Other pages use brief task-specific language: share capacity, choose who can see it, keep shipment partners informed, and build confidence through reviewed evidence. Preserve access/privacy requirements and factual badge limits at the point of decision. This editorial clarification is implemented locally; rollout remains pending.
...
Before a truck's first signal exists, Home uses a separate setup layout with the
truck identity above unobstructed instructions and a Set capacity action. It
says Not published. The first editor collects required coverage and approximate
location; a separate location-only action is available after publication.
Controls enable when their client handlers are ready. This local FEAT-CAP-001
correction preserves assigned-Driver permissions and the Private default;
owner review is accepted and rollout remains pending.
...
The submission form excludes the actor's already-Pending subject categories
and Driver-truck pairings and points to Request history. Pending evidence is
never called approved; eligible corrected evidence after More information or
rejection remains available. The server independently rejects duplicate Pending
submissions. Payment and review displays retain stored fractional ETB amounts.

Completion emails contain both labelled Tracking and Review codes and explain
email verification followed by review-code entry. Customers can complete the
existing secure flow without searching for the earlier Tracking-start email.
...
My Fleet places Add driver beside Add truck. An owner supplies a name, callback
phone and required email. The driver becomes assignable immediately, before
email verification; adding them does not confirm their email or create a login
session. Normal email-code login verifies the email and opens the same Driver
workspace with the existing truck assignment and owner-set permissions. The
caller-bound identity projection refuses unverified email identities. New drivers
start unassigned with Capacity and Tracking permissions off until the owner
chooses them. Existing provider, staff, suspended and other-fleet identities cannot
be adopted. Older pending invitations retain their verified-email acceptance,
delivery status, resend and cancellation controls. This clarified flow is verified
locally under FEAT-FLT-001 / ADR-068; production rollout remains pending.
...
Local map preview update (2026-09-21; release pending): FEAT-LST-001 and FEAT-GEO-001 cover the owner-requested location palette, compact key, city proximity, control-adjacent location notice and map gestures through informational detail cards. These clauses describe the local implementation; deployed behavior remains unchanged until separately reviewed and released.

### Dashboard and Records contract corrections (local rollout pending, 2026-09-21)

Provider Home excludes cancelled and completed work from Active Tracking, orders
Recent Tracking by latest update, and counts on-duty signals only for active
trucks. Administrative status actions retain inventory/search/page; Off Duty
is never described as Empty. Records recover a stale offset to the first bounded
page, and regular-service areas retain their area identity in list and detail.
Administrator Account explains role-based platform access without requiring a
customer subscription. These local corrections follow FEAT-ADM-001, FEAT-UIX-001
and FEAT-BIL-001; owner review and production rollout are still pending.

### Review workflow corrections (local rollout pending, 2026-09-21)

Document, rating and payment decisions retain the reviewer's queue/filter/page;
invalid or stale offsets recover a bounded first page without hiding query errors.
Truck authorization submission offers only pairings without current approval,
while preserving every badge and expired-evidence renewal. Dependent verification
selectors wait for client readiness before accepting changes. Empty-form wording
retains optional documents and distinguishes unavailable subjects from approved
categories. These local corrections follow FEAT-ADM-001 and FEAT-VER-001. The
audit records remaining payment-display precision and pending-document choice
issues separately; owner review and production rollout remain pending.

### Capacity filter drawer (local preview, release pending, 2026-09-21)

Open and verified Private capacity use the same collapsible left filter drawer.
Search, truck location, route endpoints, availability and configuration are in the
drawer; More filters opens a native modal for route tolerances/direction, signal
geometry, service areas, load type, stops and freshness. Both groups submit one
shared draft. The drawer starts open on desktop and closed on phones, can be
closed/reopened or swiped by its handle, and does not remount or resize the map.
It has no blocking backdrop. Private access/expiry/logout and scoped endpoints
remain unchanged. On phones, the chat launcher becomes compact while the drawer
is open so it cannot cover filter actions. FEAT-LST-001 / FEAT-SHR-001 govern this
owner-requested change; owner visual approval and release gates are pending.
