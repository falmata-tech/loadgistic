...
The initial Business audience is the people taking practical risks to make and trade locally: workshops, growers, farmers, processors, producers, artisans, distributors, and small manufacturers moving cartons, pallets, sacks, crates, quarter-truck, half-truck, or full-truck freight without maintaining a large private fleet. The public story explains that logistics cost and fragmented capacity can limit local, regional, and cross-regional trade, while existing empty trucks, Partial capacity, shared cargo space, and recurring routes may create more practical options. It gives equal respect to owner-operators, authorized self-managed Drivers, and small fleet transporters who need useful demand for the trucks they operate. It never promises lower prices, guaranteed work, guaranteed matches, or market access. The homepage then shows live Shipment Board and Truck Board previews. A dedicated anonymous server projection exposes structured marketplace facts such as routes, general areas, deadlines, price mode, truck configuration, capacity status, flexibility, proof signal, and freshness, while withholding identities, contacts, handles, record IDs, exact coordinates, files, and free text. Sign-in unlocks details and contact. Authenticated product language remains neutral enough for larger enterprises.
...
- the assigned Driver's current device area obscured to a Driver-chosen privacy radius and its freshness
...
When an assigned or self-managed Driver opens capacity or location-required Tracking controls, the browser requests and refreshes device location automatically while that screen remains mounted. Capacity also keeps one visible Request location or Refresh location action beside the location state, allowing the Driver to trigger a new browser reading when permission must be requested again or automatic watching has not produced a current reading. Neither workflow offers a typed current-location fallback: only the assigned Driver's obscured device point may publish truck location or throttled tracking events. A denial or unavailable device is shown plainly with guidance to enable browser permission. The browser may briefly access an exact device coordinate, but it displaces that point before application state or submission. Local, Long-distance, and Both capacity let the Driver choose a 1, 3, 5, 10, 20, or 40 km privacy area. A smaller area is labeled as more precise, and a long-distance Partial signal below 20 km carries a stronger safety warning without overriding the Driver. Tracking remains fixed at 20 km for FTL and 40 km for PTL. Only the obscured point, permitted radius, source, and human general-area label reach the server. A fleet owner may update non-location capacity facts while preserving the latest assigned-Driver location and its original timestamp, but cannot substitute the owner's phone or a typed city for the truck.
...
A Local shipment requires one reviewed locality. The owner may optionally record safe landmark labels and map-selected pickup or drop-off points for execution, including using the browser's current position for the selected private pin. Exact points never appear on Boards, pooled-shipment views, Directory, Public Profiles, or administrative summaries. The owner can review what it entered; another authorized shipper, receiver, or assigned provider receives exact points only after agreement. For Local Truck Board discovery, a Business may choose Near me: its exact device point stays in browser memory, a one-kilometer-displaced query point reaches the bounded server filter, and the map shows the Business position against permitted truck privacy circles with possible-distance ranges rather than exact truck pins.
...
Signup immediately creates an active workspace and verification remains a separate evidence-based trust layer. Every supported entity displays neutral Not verified badges or vivid, high-contrast Verified badges derived from current administrator-reviewed requests. Identity, license, address, Driver, truck, and reputation categories use distinct bright blue, orange, green, violet, cyan, and gold treatments while text and icons retain the actual meaning. Badge details explain the reviewed subject, category, date, and expiry without opening the private document. Shipment Board cards show shipment-owner evidence and published Business reviews. Truck Board cards separately show company or owner-operator, pairing-specific truck authority, and assigned-driver evidence. Directory, profile, Board, and agreement surfaces remind members to confirm current identity, authority, truck, Driver, and documents themselves; badges are not guarantees of payment, performance, cargo safety, or continuing legal authority. Administrators also receive a separate Rating Reviews queue for private low-rating investigation; publishing or dismissing a rating does not automatically suspend an account, and account or truck enforcement remains an explicit audited Operations action.

- Businesses: National ID, Business license, and Business address
- Fleet transporters: National ID, Business license, and Business address
- Self-managed drivers: National ID, Driver license, and expiring Truck authorization for the Driver-truck pairing
- Company drivers: National ID, Driver license, and expiring Truck authorization for the Driver-truck pairing, with transporter affiliation shown separately
- Trucks: historical Vehicle ownership or Owner authorization remains retained, while current operating authority is pairing-specific
