...
walk(root);
for(const file of names.filter(file=>/\.(?:js|ts|tsx)$/.test(file))){
  const source=fs.readFileSync(file,'utf8');
  if(source.includes('Deliverrex')||source.includes('MaliktBoard'))throw new Error(`Old product name in ${file}`);
  if(source.includes('node:sqlite')||source.includes('DATABASE_PATH'))throw new Error(`Retired local database dependency in ${file}`);
}
